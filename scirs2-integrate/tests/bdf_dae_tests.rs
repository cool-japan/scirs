...
use scirs2_integrate::{
    bdf_implicit_dae, bdf_semi_explicit_dae, DAEIndex, DAEOptions, DAEType, IntegrateResult,
};
...
fn test_bdf_semi_explicit_linear() {
    ...
    assert_relative_eq!(final_x, expected_x, epsilon = 1e-4);
    assert_relative_eq!(final_y, expected_y, epsilon = 1e-4);
...
fn test_bdf_implicit_dae_simple() {
    ...
    let expected_y = (-final_t).exp() + final_t;
    let expected_z = (-final_t).exp();

    assert_relative_eq!(final_y, expected_y, epsilon = 1e-4);
    assert_relative_eq!(final_z, expected_z, epsilon = 1e-4);

    // Check algebraic constraint satisfaction
