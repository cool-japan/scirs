...
use scirs2_integrate::dae::{solve_semi_explicit_dae, DAEIndex, DAEOptions, DAEType};
...
fn test_pendulum_dae() -> IntegrateResult<()> {
    ...
    // Time span: 0 to 2 seconds (short for testing)
    let t_span = [0.0, 2.0];
    ...
    for i in 0..result.t.len() {
        ...
        assert!(constraint_value.abs() < 1e-5);
    }
    ...
    assert!(energy_error < 1e-3, "Energy error: {}", energy_error);
...
fn test_linear_dae() -> IntegrateResult<()> {
    ...
    for i in 0..result.t.len() {
        let t = result.t[i];
        let x_analytical = c * (-2.0 * t).exp() + 0.5;
        let y_analytical = 0.5 - c * (-2.0 * t).exp();
        ...
        assert!(x_error < 1e-4, "X error at t={}: {}", t, x_error);
        assert!(y_error < 1e-4, "Y error at t={}: {}", t, y_error);
    }
...
fn test_implicit_linear_dae() -> IntegrateResult<()> {
    ...
    for i in 0..result.t.len() {
        ...
        assert!(x_error < 1e-3, "X error at t={}: {}", t, x_error);
        assert!(y_error < 1e-3, "Y error at t={}: {}", t, y_error);

        // Verify that the algebraic constraint is satisfied
        let constraint_value = x_numerical + y_numerical - 1.0;
        assert!(
            constraint_value.abs() < 1e-3,
...
fn test_higher_index_dae() -> IntegrateResult<()> {
    ...
    let f = |t: f64, x: ArrayView1<f64>, y: ArrayView1<f64>| -> Array1<f64> { array![y[0]] };
    ...
    // Solve the DAE system
    let result = solve_higher_index_dae(f, g, t_span, x0, y0, Some(options))?;

    // If the ProjectionMethod is used as fallback (since Pantelides is not fully implemented),
