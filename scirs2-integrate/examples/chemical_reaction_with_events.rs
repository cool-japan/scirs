...
fn main() -> IntegrateResult<()> {
    ...
    // Event functions:
    let event_funcs = vec![
        // Event 1: Detect when X reaches its maximum (dx/dt = 0)
        |_t: f64, state: ArrayView1<f64>| {
            let x = state[0];
            let y = state[1];
            a + x * x * y - b * x - x // dx/dt
        },
        // Event 2: Detect when Y reaches its maximum (dy/dt = 0)
        |_t: f64, state: ArrayView1<f64>| {
            let x = state[0];
            let y = state[1];
            b * x - x * x * y // dy/dt
        },
        // Event 3: Detect when X crosses a threshold value (e.g., X = 2.0)
        |_t: f64, state: ArrayView1<f64>| state[0] - 2.0,
        // Event 4: Detect when the system completes 5 oscillations and terminate
        |t: f64, _state: ArrayView1<f64>| t - 50.0,
    ];
