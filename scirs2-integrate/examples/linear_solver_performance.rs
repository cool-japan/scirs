...
use scirs2_integrate::ode::utils::linear_solvers::{
    auto_solve_linear_system, solve_linear_system, BandedSolver, LUDecomposition, LinearSolverType,
};
...
fn benchmark_solvers(
    ...
    for _ in 0..num_trials {
        let x = solve_linear_system(&a, &b, Some(LinearSolverType::LU), false, None, None)?;
    ...
    let start = Instant::now();
    for _ in 0..num_trials {
        let x = auto_solve_linear_system(&a, &b)?;
        if x[0] > 1e10 {
            println!("  (Large value detected)");
        }
    }
