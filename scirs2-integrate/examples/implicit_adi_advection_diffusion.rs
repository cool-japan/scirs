...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let velocity_x = 0.5; // Advection velocities
    let velocity_y = 0.5;
    ...
    let diffusion_x = |_x: f64, _y: f64, _t: f64, _u: f64| diffusion_coeff_x;
    let diffusion_y = |_x: f64, _y: f64, _t: f64, _u: f64| diffusion_coeff_y;

    // Define advection velocities
    let advection_x = |_x: f64, _y: f64, _t: f64, _u: f64| velocity_x;
    let advection_y = |_x: f64, _y: f64, _t: f64, _u: f64| velocity_y;
...
fn analyze_solution(
    ...
    let advective_cfl = dt * velocity_x.max(velocity_y) / dx;
    let diffusive_cfl = dt * (domain.diffusion_coeff_x.max(domain.diffusion_coeff_y)) / (dx * dx);
