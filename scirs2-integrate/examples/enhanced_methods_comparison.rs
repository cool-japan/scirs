...
fn create_test_problem(problem: TestProblem) -> impl Fn(f64, ArrayView1<f64>) -> Array1<f64> {
    match problem {
        TestProblem::VanDerPol(mu) => {
            move |_t: f64, y: ArrayView1<f64>| array![y[1], mu * (1.0 - y[0].powi(2)) * y[1] - y[0]]
        }
        TestProblem::Robertson => |_t: f64, y: ArrayView1<f64>| {
            array![
...
fn run_solver(
    ...
    let f = create_test_problem(problem.clone());
