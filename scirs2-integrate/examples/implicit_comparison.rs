...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let reaction_term = |_x: f64, _t: f64, u: f64| reaction_rate * u * (1.0 - u);

    // Calculate stability limits for explicit methods
    let dx = 1.0 / 100.0;
    let diffusion_limit = dx * dx / (2.0 * epsilon); // Stability limit for diffusion
    let advection_limit = dx / velocity; // CFL condition for advection
