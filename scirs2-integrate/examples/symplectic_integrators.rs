...
use scirs2_integrate::symplectic::{
    ...
    HamiltonianSystem, SeparableHamiltonian, StormerVerlet, SymplecticIntegrator,
};
...
fn simple_harmonic_oscillator() {
    ...
    println!("  Final state: q = {:.6f}, p = {:.6f}", final_q, final_p);
...
fn pendulum() {
    ...
    if let Some(h_fn) = system.hamiltonian() {
        ...
        println!("  Initial energy: {:.6f}", initial_energy);
        println!("  Final energy: {:.6f}", final_energy);
        println!(
...
fn kepler_orbit() {
    ...
    println!("  Perihelion: {:.6f}", min_radius);
    println!("  Aphelion: {:.6f}", max_radius);
    println!("  Orbital eccentricity: {:.6f}", eccentricity);

    // Check if orbit is closed
    let final_q = result.q.last().unwrap();
    let final_radius = (final_q[0] * final_q[0] + final_q[1] * final_q[1]).sqrt();

    println!("  Final radius: {:.6f}", final_radius);
    println!();
}
