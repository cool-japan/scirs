...
use scirs2_integrate::pde::{
    BoundaryCondition, BoundaryConditionType, BoundaryLocation, Domain, PDESolution,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let reaction_term = |_x: f64, _t: f64, _u: f64| reaction;
    ...
    for dt_exp in [0.01, 0.001, 0.0001] {
        ...
        let mol_solver = MOLParabolicSolver1D::new(
            domain.clone(),
            initial_condition,
            |x, t, u| diffusion_coeff(x, t, u),
            boundary_conditions.clone(),
            Some(mol_options),
        )?
        .with_advection(|x, t, u| advection_coeff(x, t, u))
        .with_reaction(|x, t, u| reaction_term(x, t, u));
    ...
    // Output summary
    let cn_info = cn_result.info.as_ref().unwrap_or(&String::new());
    let be_info = be_result.info.as_ref().unwrap_or(&String::new());
