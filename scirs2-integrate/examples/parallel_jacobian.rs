...
fn coupled_oscillators(n: usize) -> impl Fn(f64, ArrayView1<f64>) -> Array1<f64> {
...
fn reaction_diffusion(n: usize) -> impl Fn(f64, ArrayView1<f64>) -> Array1<f64> {
    move |t: f64, y: ArrayView1<f64>| {
...
fn benchmark_jacobian_strategies(
    system_name: &str,
    system_function: impl Fn(f64, ArrayView1<f64>) -> Array1<f64>,
    ...
    for &(strategy, name) in &all_strategies {
        ...
        let _ = jac_manager.update_jacobian(t, &y, &system_function, None)?;
    ...
    let start = Instant::now();
    for _ in 0..5 {
        let _ = crate::ode::utils::common::finite_difference_jacobian(
            &system_function,
            t,
            &y,
            &f_current,
