...
fn compare_with_analytical(
    ...
    let numerical = &result.u[result.u.len() - 1];

    // Compute analytical solution at final time
    let mut analytical = Array2::zeros((nx, ny));
    for i in 0..nx {
        for j in 0..ny {
            analytical[[i, j, 0]] = analytical_solution(x_grid[i], y_grid[j], final_time);
        }
    }
    ...
    for i in 0..nx {
        for j in 0..ny {
            let error = (numerical[[i, j, 0]] - analytical[[i, j, 0]]).abs();
