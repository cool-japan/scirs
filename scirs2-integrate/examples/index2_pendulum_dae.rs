...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut projection = ProjectionMethod::new(structure);
    ...
    let constraint_errors = result
        ...
        .collect::<Vec<_>>();
    let max_constraint_error = constraint_errors.iter().fold(0.0, |max, &err| max.max(err));
    ...
    let energies = result
        ...
        .collect::<Vec<_>>();
    let max_energy_error = energies.iter().fold(0.0, |max, &e| {
