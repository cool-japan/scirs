...
use scirs2_integrate::{
    BoundaryCondition, BoundaryConditionType, BoundaryLocation, Domain, SpectralElementMesh2D,
    SpectralElementOptions, SpectralElementPoisson2D,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let solver = SpectralElementPoisson2D::new(domain, source_term, bcs, Some(options))?;
    ...
    let mixed_solver =
        SpectralElementPoisson2D::new(domain, source_term, mixed_bcs, Some(mixed_options))?;
