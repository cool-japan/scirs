...
use scirs2_integrate::{
    bdf_semi_explicit_dae, DAEIndex, DAEOptions, DAEStructure, DAEType, IntegrateResult,
    ProjectionMethod,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dae_structure = DAEStructure::new(diff_vars_0.len(), alg_vars_0.len(), DAEIndex::Index2);

    // Create a projection method for constraint stabilization
    let projection = ProjectionMethod::new(dae_structure, 1e-6, 10);
