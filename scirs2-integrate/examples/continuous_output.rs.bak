...
fn write_solution_csv<F>(
    filename: &str,
    solution: &DenseSolution<F>,
    component_indices: &[usize],
    n_points: usize,
) -> IntegrateResult<()>
where
    F: std::fmt::Display + Clone + Copy,
{
    ...
    let mut file = std::fs::File::create(filename)
        .map_err(|e| scirs2_integrate::error::IntegrateError::IOError(e.to_string()))?;
...
fn pendulum_simulation() -> IntegrateResult<()> {
    ...
    let n_points = 1000;
    let (times, values) = dense_solution.dense_output(n_points)?;
    ...
    let max_deviation = energies
        .iter()
        .fold(0.0, |max, &e| max.max((e - mean_energy).abs()));
...
fn two_body_simulation() -> IntegrateResult<()> {
    ...
    let max_deviation = angular_momenta
        .iter()
        .fold(0.0, |max, &l| max.max((l - mean_l).abs()));
