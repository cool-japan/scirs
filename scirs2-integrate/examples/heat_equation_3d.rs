...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let diffusion_x = |_x: f64, _y: f64, _z: f64, _t: f64, _u: f64| d;
    let diffusion_y = |_x: f64, _y: f64, _z: f64, _t: f64, _u: f64| d;
    let diffusion_z = |_x: f64, _y: f64, _z: f64, _t: f64, _u: f64| d;
    ...
    println!(
        "ODE solver info: {}",
        mol_result.ode_info.unwrap_or_default()
    );
    ...
    for &(test_x, test_y, test_z) in &test_points {
        ...
        let numerical = final_solution[[i_z, i_y, i_x]];
    ...
    for k in 0..nz {
        for j in 0..ny {
            for i in 0..nx {
                ...
                let numerical = final_solution[[k, j, i]];
                let exact = exact_solution(x, y, z, final_time);
    ...
    {
        let u_numerical = pde_solution.values[t_idx][[center_z, center_y, center_x]];
