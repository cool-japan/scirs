...
use scirs2_core::gpu::{GpuBackend, GpuContext, GpuBuffer, GpuError};
...
fn test_large_matrix_ops(ctx: &GpuContext) -> Result<(), GpuError> {
    ...
    for (rows, cols) in sizes {
        ...
        println!("\nMatrix size: {}x{} ({} MB per matrix)", 
                 rows, cols, n_elements * 4 / (1024 * 1024));
        
        // Create test data
        let a: Vec<f32> = (0..n_elements)
            .map(|i| ((i % 1000) as f32).sin())
            .collect();
        let b: Vec<f32> = (0..n_elements)
            .map(|i| ((i % 1000) as f32).cos())
            .collect();
        ...
        let gb_processed = (n_elements as f64 * 4.0 * 2.0 * iterations as f64) 
...
fn test_massive_data_transfer(ctx: &GpuContext) -> Result<(), GpuError> {
    ...
    for size_mb in sizes_mb {
        ...
        println!("  Download: {:?} ({:.2} MB/s)", download_time, download_bandwidth);
    }
