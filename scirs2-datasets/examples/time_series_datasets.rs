...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Sampling rate: {} Hz", ecg.metadata.get("sampling_rate").unwrap_or(&"unknown".to_string()));
    println!("  Duration: {}", ecg.metadata.get("duration").unwrap_or(&"unknown".to_string()));
