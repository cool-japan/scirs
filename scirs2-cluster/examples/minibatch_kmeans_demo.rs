...
fn main() {
    ...
    println!("Generated dataset with {} samples in 2D space", data.shape()[0]);
    ...
    for &size in &dataset_sizes {
        ...
        println!("Mini-batch k-means with {} samples: {:.2?}", size, mb_duration);
...
fn generate_clustered_data(n_samples: usize, n_dim: usize) -> Array2<f64> {
    ...
    for i in 0..n_samples {
        let center = if i < n_samples / 2 { &center1 } else { &center2 };
        let dist = if i < n_samples / 2 { &cluster1_dist } else { &cluster2_dist };
...
fn calculate_cluster_agreement(labels1: &ndarray::ArrayBase<ndarray::OwnedRepr<usize>, ndarray::Dim<[usize; 1]>>, 
                             labels2: &ndarray::ArrayBase<ndarray::OwnedRepr<usize>, ndarray::Dim<[usize; 1]>>) -> f64 {
    ...
    let agreement1 = labels1.iter().zip(labels2.iter()).filter(|(&l1, &l2)| {
        if l1 == 0 { l2 == mapping1.0 } else { l2 == mapping1.1 }
    }).count() as f64 / n_samples as f64;
    
    let agreement2 = labels1.iter().zip(labels2.iter()).filter(|(&l1, &l2)| {
        if l1 == 0 { l2 == mapping2.0 } else { l2 == mapping2.1 }
    }).count() as f64 / n_samples as f64;
