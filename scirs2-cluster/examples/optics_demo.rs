...
use scirs2_cluster::density::optics::{optics, extract_dbscan_clustering, extract_xi_clusters};
...
fn main() {
    ...
    println!("First 10 points in ordering: {:?}", &result.ordering[..10.min(result.ordering.len())]);
    println!("First 10 reachability distances: {:?}", &result.reachability[..10.min(result.reachability.len())]);
...
fn print_cluster_stats(labels: &Array1<i32>) {
    ...
    let num_clusters = if *max_label >= 0 { (max_label + 1) as usize } else { 0 };
