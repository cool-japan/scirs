...
use scirs2_cluster::meanshift::{mean_shift, MeanShiftOptions, estimate_bandwidth};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data1 = array![
        ...
        // Cluster 2  
        [3.0, 3.0], [3.1, 3.1], [3.2, 3.0], [3.0, 3.2], [3.15, 3.15],
        // Cluster 3
        [6.0, 0.0], [6.1, 0.1], [6.2, 0.0], [6.0, 0.2], [6.15, 0.15],
    ];
    ...
    for (i, (center, count)) in centers.outer_iter().zip(cluster_counts.iter()).enumerate() {
        println!("  Cluster {}: center=({:.2}, {:.2}), size={}", 
    ...
    for quantile in quantiles {
        ...
        println!("\nQuantile {}: Estimated bandwidth = {:.3}", quantile, estimated_bw);
