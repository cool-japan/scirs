    ...
    F: Float + FromPrimitive + Debug + PartialOrd,
{
    ...
    let compute_centroids = store_centers == StoreCenter::Centroid || store_centers == StoreCenter::Both;
    let compute_medoids = store_centers == StoreCenter::Medoid || store_centers == StoreCenter::Both;
    ...
    F: Float + FromPrimitive + Debug + PartialOrd,
{
    ...
    let single_linkage_tree = match &hdbscan_result.single_linkage_tree {
        Some(tree) => tree,
        None => return Err(ClusteringError::InvalidInput(
    ...
    let lambdas: Vec<F> = single_linkage_tree.distances.iter()
        .map(|&d| {
            if d > F::zero() {
                F::one() / d
            } else {
    ...
    for i in 0..lambdas.len() {
        ...
        // If both are internal nodes, merge all points in both subtrees
            ...
            if !left_points.is_empty() && !right_points.is_empty() {
                ...
                for &point in &right_points {
                    if point >= 0 && point < n_samples as i32 && left_rep >= 0 && left_rep < n_samples as i32 {
...
fn get_leaves(
    node: i32,
    tree: &SingleLinkageTree<impl Float>,
    n_samples: i32,
) -> Vec<i32> {
...
fn mutual_reachability_distance<F: Float>(
    distance: F,
    core_dist1: F,
    core_dist2: F,
) -> F {
    ...
    F: Float + FromPrimitive + Debug + PartialOrd,
{
    ...
    let mut lambdas: Vec<F> = single_linkage_tree.distances.iter()
        .map(|&d| {
            if d > F::zero() {
                F::one() / d
            } else {
    ...
    F: Float + FromPrimitive + Debug + PartialOrd,
{
    ...
    let root: i32 = condensed_tree.parent.iter()
        .fold(0, |max, &p| max.max(p));
    ...
    if method == ClusterSelectionMethod::Leaf {
        ...
        let leaf_clusters: Vec<i32> = is_leaf.iter()
            .filter(|&&node| node >= 0)
            .cloned()
            .collect();
        
        // Assign points to clusters
        let (labels, probabilities) = assign_points_to_clusters(
            condensed_tree,
            &leaf_clusters,
            root,
        )?;
    ...
    let mut nodes_by_lambda: Vec<(usize, F)> = condensed_tree.lambda_val.iter()
        .enumerate()
    ...
    for (idx, _) in nodes_by_lambda {
        ...
        if child >= 0 {
            ...
            let parent_lambda = condensed_tree.lambda_val.iter()
    ...
    while !to_process.is_empty() {
        ...
        let children: Vec<i32> = condensed_tree.parent.iter()
        ...
        let max_child_stability = children.iter()
            .filter(|&&c| c >= 0)
    ...
    if selected_clusters_vec.is_empty() && allow_single_cluster {
        ...
        let highest_child = condensed_tree.child.iter()
            .filter(|&&c| c >= 0 && c != root)
            .cloned()
            .max()
            .unwrap_or(-1);
    ...
    F: Float + FromPrimitive + Debug + PartialOrd,
{
    ...
    let max_point_idx = condensed_tree.child.iter()
        .filter(|&&c| c < 0)
        .map(|&c| -c - 1)
        .max()
    ...
    for point_idx in 0..n_samples {
        ...
        let point_edges: Vec<(i32, F)> = condensed_tree.child.iter()
            .zip(condensed_tree.parent.iter())
        ...
        for (node, lambda) in point_edges {
            ...
            loop {
                ...
                let parent_edges: Vec<(i32, F)> = condensed_tree.child.iter()
                    .zip(condensed_tree.parent.iter())
                    .zip(condensed_tree.lambda_val.iter())
    ...
    let remap: std::collections::HashMap<i32, i32> = unique_labels.iter()
        .enumerate()
        .map(|(i, &label)| (label, i as i32))
        .collect();
    
    // Remap labels
    let remapped_labels: Vec<i32> = labels.iter()
        .map(|&label| {
            if label >= 0 {
