...
pub fn estimate_bandwidth<T: Float + Display + FromPrimitive + Send + Sync + 'static>(
    ...
    for row in data.rows() {
        for &val in row.iter() {
            if !val.is_finite() {
                return Err(ClusteringError::InvalidInput("Input contains non-finite values".to_string()));
            }
        }
    }
    ...
    let n_neighbors = (T::from(data.nrows()).unwrap() * quantile).to_usize()
        .unwrap_or(1)
        .max(1);
    ...
    for i in (0..data.nrows()).step_by(batch_size) {
        ...
        for row in batch.rows() {
            let (_, distances) = kdtree.query(&row.to_vec(), n_neighbors + 1)
                .map_err(|e| ClusteringError::ComputationError(format!("Failed to query KDTree: {}", e)))?;
            
            if distances.len() > 1 {
                // Skip the first distance (to itself, which is 0) and take the max
                let max_dist_val = distances.iter()
                    .skip(1)
...
fn mean_shift_single_seed<T: Float + Display + std::iter::Sum + FromPrimitive + Send + Sync + 'static + ndarray::ScalarOperand>(
    seed: ArrayView1<T>,
    data: &ArrayView2<T>,
    bandwidth: T,
    max_iter: usize,
...
/// ```
pub fn mean_shift<T: Float + Display + std::iter::Sum + FromPrimitive + Send + Sync + 'static + ndarray::ScalarOperand>(
    data: &ArrayView2<T>,
    ...
    Ok((model.cluster_centers().to_owned(), model.labels().to_owned()))
}
...
impl<T: Float + Display + std::iter::Sum + FromPrimitive + Send + Sync + 'static + ndarray::ScalarOperand> MeanShift<T> {
    ...
    pub fn fit(&mut self, data: &ArrayView2<T>) -> Result<&mut Self, ClusteringError> {
        ...
        sorted_by_intensity.sort_by(|a, b| {
            b.1.cmp(&a.1).then_with(|| {
                a.0.0.iter().zip(b.0.0.iter()).find_map(|(a_val, b_val)| {
        ...
        let kdtree = KDTree::<_, EuclideanDistance<T>>::new(&sorted_centers)
            .map_err(|e| ClusteringError::ComputationError(format!("Failed to build KDTree: {}", e)))?;
        
        for i in 0..sorted_centers.nrows() {
            if unique[i] {
                let (indices, _) = kdtree.query_radius(&sorted_centers.row(i).to_vec(), bandwidth)
                    .map_err(|e| ClusteringError::ComputationError(format!("Failed to query KDTree: {}", e)))?;
        ...
        let unique_indices: Vec<_> = unique.iter().enumerate()
        ...
        let kdtree = KDTree::<_, EuclideanDistance<T>>::new(&cluster_centers)
            .map_err(|e| ClusteringError::ComputationError(format!("Failed to build KDTree: {}", e)))?;
        ...
        for i in (0..n_samples).step_by(batch_size) {
            ...
            for (row_idx, row) in batch.rows().into_iter().enumerate() {
                ...
                let (indices, distances) = kdtree.query(&row.to_vec(), 1)
                    .map_err(|e| ClusteringError::ComputationError(format!("Failed to query KDTree: {}", e)))?;
    ...
    pub fn cluster_centers(&self) -> &Array2<T> {
        self.cluster_centers_.as_ref().expect("Model has not been fitted yet")
    }
    
    /// Get labels assigned to each data point.
    pub fn labels(&self) -> &Array1<i32> {
        self.labels_.as_ref().expect("Model has not been fitted yet")
    }
    ...
    pub fn predict(&self, data: &ArrayView2<T>) -> Result<Array1<i32>, ClusteringError> {
        let centers = self.cluster_centers_.as_ref()
            .ok_or_else(|| ClusteringError::InvalidState("Model has not been fitted yet".to_string()))?;
        ...
        let kdtree = KDTree::<_, EuclideanDistance<T>>::new(centers)
            .map_err(|e| ClusteringError::ComputationError(format!("Failed to build KDTree: {}", e)))?;
        ...
        for i in (0..n_samples).step_by(batch_size) {
            ...
            for (row_idx, row) in batch.rows().into_iter().enumerate() {
                let (indices, _) = kdtree.query(&row.to_vec(), 1)
                    .map_err(|e| ClusteringError::ComputationError(format!("Failed to query KDTree: {}", e)))?;
...
mod tests {
    ...
    fn test_get_bin_seeds() {
        let data = array![
            [1.0, 1.0], [1.4, 1.4], [1.8, 1.2], 
            [2.0, 1.0], [2.1, 1.1], [0.0, 0.0]
        ];
