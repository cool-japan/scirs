...
use crate::vq::{kmeans, KMeansOptions, KMeansInit};
...
where
    F: Float + FromPrimitive + Debug + PartialOrd + 'static + std::iter::Sum + 
       std::ops::AddAssign + std::ops::SubAssign + std::ops::MulAssign + std::ops::DivAssign + std::ops::RemAssign,
{
    ...
    let embedding = if opts.normalized_laplacian {
        ...
        eigenvectors.slice(s![.., 1..(actual_n_clusters+1)]).to_owned()
    };
    ...
    let (_, labels) = kmeans(normalized_embedding.view(), actual_n_clusters, Some(kmeans_opts))?;
...
mod tests {
    ...
    fn test_spectral_clustering_basic() {
        ...
        let silhouette = silhouette_score(data.view(), labels.mapv(|x| x as i32).view(), None).unwrap();
        assert!(silhouette > 0.8, "Silhouette score should be high for well-separated clusters");
    }
    ...
    fn test_spectral_clustering_ring() {
        ...
        let data = Array2::from_shape_vec((16, 2), vec![
            // First ring (8 points)
            1.0, 0.0,
            0.7, 0.7,
            0.0, 1.0,
