    ...
    F: Float + Sum + Debug,
{
    ...
    // Adjust vh dimensions for non-full matrices
    let vh_rows = if full_matrices { n } else { k };
    let mut vh_final = Array2::zeros((vh_rows, n));
    vh_final.slice_mut(ndarray::s![..vh_rows.min(vt.nrows()), ..]).assign(&vt.slice(ndarray::s![..vh_rows.min(vt.nrows()), ..]));
    
    Ok(ComplexSVDDecomposition { u, s, vh: vh_final })
}
        ...
        if norm_sq > tolerance {
            ...
            if v_norm_sq > tolerance {
                ...
                for j in col..n {
                    ...
                    let factor = Complex::<F>::new(F::from(2.0).unwrap() / v_norm_sq, F::zero()) * sum;
    ...
    while !converged && iterations < max_iterations {
        ...
        if !converged {
            ...
            if n >= 2 {
                ...
                let discriminant = trace * trace - Complex::<F>::new(F::from(4.0).unwrap(), F::zero()) * det;
                let sqrt_disc = discriminant.sqrt();
                
                let eig1 = (trace + sqrt_disc) / Complex::<F>::new(F::from(2.0).unwrap(), F::zero());
                let eig2 = (trace - sqrt_disc) / Complex::<F>::new(F::from(2.0).unwrap(), F::zero());
        ...
        for k in 0..(n - 1) {
            ...
            if r > F::zero() {
                ...
                for i in 0..n {
                    ...
                    q_total[[i, k]] = Complex::<F>::new(c, F::zero()) * q_ik + Complex::<F>::new(sn, F::zero()) * q_ik1;
                    q_total[[i, k + 1]] = Complex::<F>::new(-sn, F::zero()) * q_ik + Complex::<F>::new(c, F::zero()) * q_ik1;
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_complex_qr() {
        ...
        println!("Q shape: {:?}, R shape: {:?}", qr_result.q.shape(), qr_result.r.shape());
        ...
        for i in 0..m {
            for j in 0..m {
                ...
                assert!(
                    diff < 1e-10,
                    "Q is not unitary at position ({}, {}): diff = {}", i, j, diff
                );
            }
        }
        ...
        for i in 0..qr_result.r.nrows() {
            for j in 0..i.min(qr_result.r.ncols()) {
                assert!(
                    ...
                    "R is not upper triangular at ({}, {})", i, j
                );
            }
        }
    }
