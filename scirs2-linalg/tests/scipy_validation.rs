...
use scirs2_linalg::{det, inv, lu, qr, svd, cholesky, matrix_norm, solve};
...
fn test_svd_against_scipy() {
    ...
    // Create a proper test for a square matrix where SVD reconstruction works
    let a_square = array![
        [3.0_f64, 1.0],
        [1.0, 2.0]
    ];
    
    let (u_sq, s_sq, vt_sq) = svd(&a_square.view(), false).unwrap();
    
    // Reconstruct the matrix for square case
    let mut s_diag = Array2::zeros((u_sq.ncols(), vt_sq.nrows()));
    for i in 0..s_sq.len().min(s_diag.nrows()).min(s_diag.ncols()) {
        s_diag[[i, i]] = s_sq[i];
    }
    
    let svd_product = u_sq.dot(&s_diag).dot(&vt_sq);
    assert_abs_diff_eq!(svd_product, a_square, epsilon = TOLERANCE);
}
