...
use scirs2_neural::layers::{Dense, Dropout, Layer, LayerNorm};
use scirs2_neural::models::Sequential;
use scirs2_neural::serialization::SaveLoad;
use std::path::Path;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = model.forward(&input.into_dyn())?;
    ...
    let loaded_model = Sequential::<f32>::load(model_path)?;
