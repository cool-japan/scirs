...
use scirs2_neural::{
    models::architectures::{AttentionType, RNNCellType, Seq2Seq, Seq2SeqConfig},
    prelude::*,
};
...
fn main() -> Result<()> {
    ...
    for elem in input_seq.iter_mut() {
        *elem = (rand::random::<f32>() * (src_vocab_size as f32 - 1.0)).floor();
    }
    ...
    for elem in target_seq.iter_mut() {
        *elem = (rand::random::<f32>() * (tgt_vocab_size as f32 - 1.0)).floor();
    }

    // 1. Create a basic translation model
    println!("\nCreating Basic Translation Model...");
    let translation_model = Seq2Seq::create_translation_model(
        src_vocab_size,
        tgt_vocab_size,
    ...
    let custom_model = Seq2Seq::new(custom_config)?;
