...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let encoder_output = transformer.forward(&src_embeddings.into_dyn())?;
    ...
    let output_train =
        transformer.forward_train(&src_embeddings.into_dyn(), &tgt_embeddings.into_dyn())?;
    ...
    let output_inference =
        transformer.forward_inference(&src_embeddings.into_dyn(), &first_token.into_dyn())?;
