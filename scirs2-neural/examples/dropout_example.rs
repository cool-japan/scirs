...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hidden_output = dense1.forward(&input_data.into_dyn())?;
    ...
    let out_channels = 16;
    let conv1 = Conv2D::new(in_channels, out_channels, 3, 1, PaddingMode::Same, &mut rng)?;
    ...
    let conv_output = conv1.forward(&image_input.into_dyn())?;
