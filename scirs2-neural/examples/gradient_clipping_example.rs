...
fn create_sample_gradients() -> Vec<Array<f32, IxDyn>> {
    ...
    let grad1 = Array::from_shape_vec(
        IxDyn(&[3, 2]), 
        vec![0.1, 0.2, 1.5, -0.8, 0.7, -1.2]
    ).unwrap();
    gradients.push(grad1);
    
    // First layer gradients (bias)
    let grad2 = Array::from_shape_vec(
        IxDyn(&[3]), 
        vec![0.5, -1.8, 0.9]
    ).unwrap();
    ...
    let grad3 = Array::from_shape_vec(
        IxDyn(&[2, 3]), 
        vec![0.3, -0.4, 1.2, -1.5, 0.8, 0.6]
    ).unwrap();
    gradients.push(grad3);
    
    // Second layer gradients (bias)
    let grad4 = Array::from_shape_vec(
        IxDyn(&[2]), 
        vec![0.2, -0.9]
    ).unwrap();
