...
impl<F: Float + Debug> Loss<F> for FocalLoss {
    ...
    fn backward(
        ...
        Zip::from(&mut gradients)
            .and(predictions)
            .and(targets)
            .enumerate()
            .for_each(|(idx, grad, &p, &t)| {
                let p_safe = p.max(epsilon).min(F::one() - epsilon);

                // For simplicity, use a multi-dimensional index
                let alpha = if let Some(ref alpha_per_class) = self.alpha_per_class {
                    // For multi-dimensional arrays, extract the class index
                    let class_idx = if predictions.ndim() > 1 {
                        idx % predictions.shape()[1]
                    } else {
                        idx
                    };
                    F::from(alpha_per_class[class_idx]).unwrap_or(F::one())
                } else if let Some(a) = self.alpha {
                    F::from(a).unwrap_or(F::one())
                } else {
                    F::one()
                ...
                if t > F::zero() {
                    // For the target class, the gradient is more complex:
                    // d(FL)/d(p_t) = -alpha * gamma * (1-p_t)^(gamma-1) * log(p_t) - alpha * (1-p_t)^gamma * (1/p_t)
                    ...
                    *grad = (term1 + term2) * t / n;
                } else {
                    // For non-target classes, the gradient is simplified
                    *grad = F::zero();
                }
            });
