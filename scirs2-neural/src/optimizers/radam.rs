...
impl<F: Float + ScalarOperand + Debug> RAdam<F> {
    ...
    pub fn new(learning_rate: F, beta1: F, beta2: F, epsilon: F, weight_decay: F) -> Self {
        ...
        let five_thousand = F::from(5000.0).unwrap();
...
impl<F: Float + ScalarOperand + Debug> Optimizer<F> for RAdam<F> {
    fn update(
        ...
        let rho_t = self.rho_inf
            - two * (self.t as f64).into_f64() * beta2_pow_t.to_f64().unwrap()
                / (F::one() - beta2_pow_t).to_f64().unwrap();

        // Convert back to F type
        let rho_t = F::from(rho_t).unwrap();
