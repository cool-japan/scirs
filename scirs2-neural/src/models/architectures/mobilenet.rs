...
impl MobileNetConfig {
    ...
    pub fn mobilenet_v3_small(input_channels: usize, num_classes: usize) -> Self {
        ...
        // [exp_size, out_ch, kernel, stride, use_se, activation, use_res]
        let configs = vec![
            (1, 16, 3, 2, true, "relu", false),
            (4, 24, 3, 2, false, "relu", false),
            (3, 24, 3, 1, false, "relu", true),
            (3, 40, 5, 2, true, "hard_swish", false),
            (3, 40, 5, 1, true, "hard_swish", true),
            (3, 40, 5, 1, true, "hard_swish", true),
            (6, 80, 3, 2, false, "hard_swish", false),
            (2.5, 80, 3, 1, false, "hard_swish", true),
            (2.3, 80, 3, 1, false, "hard_swish", true),
            (2.3, 80, 3, 1, false, "hard_swish", true),
            (6, 112, 3, 1, true, "hard_swish", false),
            (6, 112, 3, 1, true, "hard_swish", true),
            (6, 160, 5, 2, true, "hard_swish", false),
            (6, 160, 5, 1, true, "hard_swish", true),
            (6, 160, 5, 1, true, "hard_swish", true),
        ];
    ...
    pub fn mobilenet_v3_large(input_channels: usize, num_classes: usize) -> Self {
        ...
        // [exp_size, out_ch, kernel, stride, use_se, activation, use_res]
        let configs = vec![
            (1, 16, 3, 1, false, "relu", false),
            (4, 24, 3, 2, false, "relu", false),
            (3, 24, 3, 1, false, "relu", true),
            (3, 40, 5, 2, true, "relu", false),
            (3, 40, 5, 1, true, "relu", true),
            (3, 40, 5, 1, true, "relu", true),
            (6, 80, 3, 2, false, "hard_swish", false),
            (2.5, 80, 3, 1, false, "hard_swish", true),
            (2.3, 80, 3, 1, false, "hard_swish", true),
...
struct SqueezeExcitation<F: Float + Debug + ScalarOperand> {
    /// Input channels
...
impl<F: Float + Debug + ScalarOperand> SqueezeExcitation<F> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for SqueezeExcitation<F> {
...
/// Activation function
fn get_activation<F: Float + Debug + ScalarOperand>(name: &str) -> Box<dyn Fn(F) -> F> {
    match name {
...
struct InvertedResidualBlock<F: Float + Debug + ScalarOperand> {
    /// Block configuration
    ...
    /// Activation function
    activation: Box<dyn Fn(F) -> F>,
}

impl<F: Float + Debug + ScalarOperand> InvertedResidualBlock<F> {
    ...
    pub fn new(config: ConvBlockConfig, width_multiplier: f64) -> Result<Self> {
        ...
        let (expand_conv, expand_bn) = if expand_ratio != 1 {
            ...
            // Batch normalization
            let bn = BatchNorm::new(expanded_channels, 1e-3, 0.01, true)?;
        ...
        let depthwise_conv = Conv2D::new(
            ...
            stride,
            1,
            expanded_channels, // groups = expanded_channels for depthwise
            PaddingMode::Same,
            true,
        )?;

        // Depthwise batch normalization
        let depthwise_bn = BatchNorm::new(expanded_channels, 1e-3, 0.01, true)?;
        ...
        // Projection batch normalization
        let project_bn = BatchNorm::new(output_channels, 1e-3, 0.01, true)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for InvertedResidualBlock<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
struct DepthwiseSeparableConv<F: Float + Debug + ScalarOperand> {
    /// Block configuration
    config: ConvBlockConfig,
    /// Depthwise convolution
    depthwise_conv: Conv2D<F>,
...
impl<F: Float + Debug + ScalarOperand> DepthwiseSeparableConv<F> {
    ...
    pub fn new(config: ConvBlockConfig, width_multiplier: f64) -> Result<Self> {
        ...
        let depthwise_conv = Conv2D::new(
            ...
            stride,
            1,
            input_channels, // groups = input_channels for depthwise
            PaddingMode::Same,
            true,
        )?;

        // Depthwise batch normalization
        let depthwise_bn = BatchNorm::new(input_channels, 1e-3, 0.01, true)?;
        ...
        // Pointwise batch normalization
        let pointwise_bn = BatchNorm::new(output_channels, 1e-3, 0.01, true)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for DepthwiseSeparableConv<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        // Depthwise convolution
...
/// MobileNet implementation
pub struct MobileNet<F: Float + Debug + ScalarOperand> {
    /// Model configuration
    ...
    stem_activation: Box<dyn Fn(F) -> F>,
}

impl<F: Float + Debug + ScalarOperand> MobileNet<F> {
    ...
    pub fn new(config: MobileNetConfig) -> Result<Self> {
        ...
        let stem_bn = BatchNorm::new(stem_channels, 1e-3, 0.01, true)?;
        ...
        // Classifier
        let classifier = Dense::new(last_channels, num_classes, None, true)?;

        // Dropout
        let dropout = Dropout::new(F::from(config.dropout_rate).unwrap());
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for MobileNet<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        // Check input shape
        let shape = input.shape();
