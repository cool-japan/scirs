...
use crate::layers::{
    Conv2D, Dense, Dropout, Layer, Sequential, LayerNorm2D, GlobalAvgPool2D,
};
...
#[derive(Debug, Clone)]
pub struct ConvNeXtBlock<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> ConvNeXtBlock<F> {
    ...
    pub fn new(channels: usize, layer_scale_init_value: f64, drop_path_prob: f64) -> Result<Self> {
        ...
        let norm = LayerNorm2D::<F>::new(channels, 1e-6, Some("norm"))?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for ConvNeXtBlock<F> {
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        // Apply layer scale
        let shape = x.shape();
        let view = x.into_shape((shape[0], shape[1], shape[2] * shape[3]))?;
        let scaled = view.clone() * &self.gamma;
        x = scaled.into_shape(shape).unwrap();
    ...
    fn set_training(&mut self, training: bool) {
        ...
        self.gelu.set_training(training);
    }
...
#[derive(Debug, Clone)]
pub struct ConvNeXtDownsample<F: Float + Debug + ScalarOperand> {
    /// Layer normalization
    pub norm: LayerNorm2D<F>,
...
impl<F: Float + Debug + ScalarOperand> ConvNeXtDownsample<F> {
    /// Create a new ConvNeXtDownsample
    pub fn new(in_channels: usize, out_channels: usize, stride: usize) -> Result<Self> {
        let norm = LayerNorm2D::<F>::new(in_channels, 1e-6, Some("downsample_norm"))?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for ConvNeXtDownsample<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
...
/// ConvNeXt stage
#[derive(Debug, Clone)]
pub struct ConvNeXtStage<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> ConvNeXtStage<F> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for ConvNeXtStage<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
...
/// ConvNeXt model
#[derive(Debug, Clone)]
pub struct ConvNeXt<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> ConvNeXt<F> {
    ...
    pub fn new(config: ConvNeXtConfig) -> Result<Self> {
        ...
        let head = if config.include_top {
            ...
            head_seq.add(LayerNorm2D::<F>::new(
            ...
            if let Some(dropout_rate) = config.dropout_rate {
                head_seq.add(Dropout::<F>::new(dropout_rate, Some("head_dropout"))?);
            }
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for ConvNeXt<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
