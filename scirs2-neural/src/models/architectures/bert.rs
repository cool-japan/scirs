...
struct BertEmbeddings<F: Float + Debug + ScalarOperand> {
    /// Token embeddings
...
impl<F: Float + Debug + ScalarOperand> BertEmbeddings<F> {
    ...
    pub fn new(config: &BertConfig) -> Result<Self> {
        ...
        let layer_norm = LayerNorm::new(config.hidden_size, layer_norm_eps)?;

        // Dropout
        let dropout_prob = F::from(config.hidden_dropout_prob).unwrap();
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertEmbeddings<F> {
...
struct BertSelfAttention<F: Float + Debug + ScalarOperand> {
    /// Number of attention heads
    num_attention_heads: usize,
...
impl<F: Float + Debug + ScalarOperand> BertSelfAttention<F> {
    /// Create BERT self-attention layer
    pub fn new(config: &BertConfig) -> Result<Self> {
        let attention_head_size = config.hidden_size / config.num_attention_heads;

        // Multi-head attention
        let attention_dropout = F::from(config.attention_probs_dropout_prob).unwrap();
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertSelfAttention<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
struct BertIntermediate<F: Float + Debug + ScalarOperand> {
    /// Dense layer
    dense: Dense<F>,
    /// Activation function
...
impl<F: Float + Debug + ScalarOperand> BertIntermediate<F> {
    /// Create BERT intermediate layer
    pub fn new(config: &BertConfig) -> Result<Self> {
        // Dense layer
        let dense = Dense::new(config.hidden_size, config.intermediate_size, None, true)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertIntermediate<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        // Apply dense layer
...
/// BERT output layer
struct BertOutput<F: Float + Debug + ScalarOperand> {
    /// Dense layer
    dense: Dense<F>,
    /// Layer normalization
    layer_norm: LayerNorm<F>,
...
impl<F: Float + Debug + ScalarOperand> BertOutput<F> {
    /// Create BERT output layer
    pub fn new(config: &BertConfig) -> Result<Self> {
        // Dense layer
        let dense = Dense::new(config.intermediate_size, config.hidden_size, None, true)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertOutput<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
struct BertAttention<F: Float + Debug + ScalarOperand> {
    /// Self attention
...
impl<F: Float + Debug + ScalarOperand> BertAttention<F> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertAttention<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
/// BERT layer
struct BertLayer<F: Float + Debug + ScalarOperand> {
    /// Attention block
...
impl<F: Float + Debug + ScalarOperand> BertLayer<F> {
    /// Create BERT layer
    pub fn new(config: &BertConfig) -> Result<Self> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertLayer<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        let attention_output = self.attention.forward(input)?;
...
/// BERT encoder
struct BertEncoder<F: Float + Debug + ScalarOperand> {
    /// BERT layers
    layers: Vec<BertLayer<F>>,
}

impl<F: Float + Debug + ScalarOperand> BertEncoder<F> {
    /// Create BERT encoder
    pub fn new(config: &BertConfig) -> Result<Self> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertEncoder<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
/// BERT pooler
struct BertPooler<F: Float + Debug + ScalarOperand> {
    /// Dense layer
    dense: Dense<F>,
    /// Activation function
    activation_fn: Box<dyn Fn(F) -> F>,
...
impl<F: Float + Debug + ScalarOperand> BertPooler<F> {
    /// Create BERT pooler
    pub fn new(config: &BertConfig) -> Result<Self> {
        // Dense layer
        let dense = Dense::new(config.hidden_size, config.hidden_size, None, true)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertPooler<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
pub struct BertModel<F: Float + Debug + ScalarOperand> {
    /// Embeddings layer
...
impl<F: Float + Debug + ScalarOperand> BertModel<F> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BertModel<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
