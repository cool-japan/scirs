...
use crate::layers::{
    Dense, Dropout, Embedding, Layer, GRU, LSTM, RNN,
};
...
#[derive(Debug, Clone)]
pub struct Attention<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> Attention<F> {
    ...
    pub fn new(
        ...
        let decoder_projection = Dense::<F>::new(
            decoder_dim,
            attention_dim,
            None,
            None,
            Some("attention_decoder_proj"),
        )?;
        ...
        let combined_projection =
            Dense::<F>::new(combined_dim, 1, None, None, Some("attention_score_proj"))?;
    ...
    pub fn forward(
        ...
        let attention_scores = match self.attention_type {
            AttentionType::Additive => {
                ...
                let encoder_projected = if let Some(ref proj) = self.encoder_projection {
                    ...
                    let flat_encoder = encoder_outputs
                        .clone()
                        .into_shape((batch_size * seq_len, encoder_dim))?;
                    let projected = proj.forward(&flat_encoder)?;
                    projected.into_shape((batch_size, seq_len, projected.shape()[1]))?
                } else {
                ...
                let expanded_decoder = decoder_projected.clone().into_shape((
                    batch_size,
                    1,
                ...
                let flat_tanh = tanh
                    .clone()
                    .into_shape((batch_size * seq_len, tanh.shape()[2]))?;
                let scores = self.combined_projection.forward(&flat_tanh)?;
                scores.into_shape((batch_size, seq_len))?
            }
            AttentionType::Multiplicative => {
                ...
                for b in 0..batch_size {
                    ...
                    for s in 0..seq_len {
                        ...
                        scores[[b, s]] = decoder_slice.dot(&encoder_slice);
                    }
                }
            ...
            AttentionType::General => {
                ...
                let weight_matrix = decoder_projected.clone();
                ...
                for b in 0..batch_size {
                    ...
                    for s in 0..seq_len {
                        let encoder_slice = encoder_outputs.slice(ndarray::s![b, s, ..]);
                        scores[[b, s]] = weight.dot(&encoder_slice);
                    }
                }
        ...
        let attention_weights_expanded = attention_weights
            .clone()
            .into_shape((batch_size, seq_len, 1))?;
        ...
        let decoder_and_context =
            ndarray::stack(Axis(1), &[context.view(), decoder_state.clone()])?;
        let flattened = decoder_and_context
            .into_shape((batch_size, context.shape()[1] + decoder_state.shape()[1]))?;

        // Project combined vector
        let output = self.output_projection.forward(&flattened)?;

        Ok((output, attention_weights))
    }
}

impl<F: Float + Debug + ScalarOperand> Layer<F> for Attention<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        Err(Error::InvalidInput("Attention layer requires separate decoder state and encoder outputs. Use the dedicated forward method.".to_string()))
    }
...
#[derive(Debug, Clone)]
pub struct Seq2SeqEncoder<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> Seq2SeqEncoder<F> {
    ...
    pub fn new(
        ...
        let embedding =
            Embedding::<F>::new(vocab_size, embedding_dim, None, Some("encoder_embedding"))?;
        ...
        for i in 0..num_layers {
            ...
            let rnn: Box<dyn Layer<F> + Send + Sync> = match cell_type {
                RNNCellType::SimpleRNN => {
                    ...
                    let rnn = RNN::<F>::new(config, Some(&format!("encoder_rnn_{}", i)))?;

                    if bidirectional {
                        let brnn = Bidirectional::new(Box::new(rnn), None)?;
                        Box::new(brnn)
                    } else {
                        Box::new(rnn)
                    }
                ...
                RNNCellType::LSTM => {
                    ...
                    let lstm = LSTM::<F>::new(config, Some(&format!("encoder_lstm_{}", i)))?;

                    if bidirectional {
                        let blstm = Bidirectional::new(Box::new(lstm), None)?;
                        Box::new(blstm)
                    } else {
                        Box::new(lstm)
                    }
                ...
                RNNCellType::GRU => {
                    ...
                    let gru = GRU::<F>::new(config, Some(&format!("encoder_gru_{}", i)))?;

                    if bidirectional {
                        let bgru = Bidirectional::new(Box::new(gru), None)?;
                        Box::new(bgru)
                    } else {
                        Box::new(gru)
                    }
        ...
        let dropout = if let Some(rate) = dropout_rate {
            if rate > 0.0 {
                Some(Dropout::<F>::new(rate, Some("encoder_dropout"))?)
            } else {
                None
            }
        } else {
        ...
        for layer in &self.rnn_layers {
            ...
            if self.bidirectional {
                ...
                let sequences = output
                    ...
                    .into_shape((output.shape()[0], output.shape()[2], output.shape()[3]))?;

                let state = output
                    .slice_axis(Axis(1), ndarray::Slice::from(1..2))
                    .into_shape((output.shape()[0], output.shape()[3]))?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for Seq2SeqEncoder<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
#[derive(Debug, Clone)]
pub struct Seq2SeqDecoder<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> Seq2SeqDecoder<F> {
    ...
    pub fn new(
        ...
        // Create embedding layer
        let embedding =
            Embedding::<F>::new(vocab_size, embedding_dim, None, Some("decoder_embedding"))?;
        ...
        for i in 0..num_layers {
            ...
            let rnn: Box<dyn Layer<F> + Send + Sync> = match cell_type {
                RNNCellType::SimpleRNN => {
                    ...
                    let rnn = RNN::<F>::new(config, Some(&format!("decoder_rnn_{}", i)))?;
                    Box::new(rnn)
                }
                RNNCellType::LSTM => {
                    let config = LSTMConfig {
                    ...
                    let lstm = LSTM::<F>::new(config, Some(&format!("decoder_lstm_{}", i)))?;
                    Box::new(lstm)
                }
                RNNCellType::GRU => {
                    let config = GRUConfig {
                    ...
                    let gru = GRU::<F>::new(config, Some(&format!("decoder_gru_{}", i)))?;
                    Box::new(gru)
                }
            };
        ...
        let output_projection = Dense::<F>::new(
            hidden_dim,
            vocab_size,
            Some(Box::new(Softmax::<F>::new())),
            None,
            Some("decoder_output_proj"),
        )?;

        // Create dropout layer if needed
        let dropout = if let Some(rate) = dropout_rate {
            if rate > 0.0 {
                Some(Dropout::<F>::new(rate, Some("decoder_dropout"))?)
            } else {
                None
            }
        } else {
    ...
    pub fn forward_step(
        ...
        for (i, layer) in self.rnn_layers.iter().enumerate() {
            ...
            let output = if let Some(state) = prev_state {
                ...
                let initial_state =
                    state
                        .clone()
                        .into_shape((state.shape()[0], 1, state.shape()[1]))?;

                let combined_input = ndarray::stack(Axis(1), &[x.view(), initial_state.view()])?;
                layer.forward(&combined_input)?
            } else {
                layer.forward(&x)?
            };
        ...
        let final_output = if let Some(ref attention) = self.attention {
            if let Some(encoder_out) = encoder_outputs {
                ...
                let last_hidden = x.into_shape((batch_size, hidden_size))?;

                // Apply attention
                let (attentional_hidden, _) = attention.forward(&last_hidden, encoder_out)?;
            ...
            // Project to vocabulary size
            self.output_projection.forward(&last_hidden)?
        };
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for Seq2SeqDecoder<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
#[derive(Debug, Clone)]
pub struct Seq2Seq<F: Float + Debug + ScalarOperand> {
    /// Encoder component
...
impl<F: Float + Debug + ScalarOperand> Seq2Seq<F> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for Seq2Seq<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
