...
use crate::layers::{
    Dense, Layer, LayerNorm, Sequential,
};
...
#[derive(Debug, Clone)]
pub struct CLIPTextEncoder<F: Float + Debug + ScalarOperand> {
    /// Token embedding
...
impl<F: Float + Debug + ScalarOperand> CLIPTextEncoder<F> {
    ...
    pub fn new(config: CLIPTextConfig, projection_dim: usize) -> Result<Self> {
        ...
        for i in 0..config.num_layers {
            encoder_layers.push(TransformerEncoderLayer::<F>::new(
                ...
                Some(&format!("encoder_layer_{}", i)),
            )?);
        }

        // Layer normalization
        let layer_norm = LayerNorm::<F>::new(
            config.hidden_size,
            config.layer_norm_eps,
            Some("layer_norm"),
        )?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for CLIPTextEncoder<F> {
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        // Apply projection
        let output = self.projection.forward(&cls_token.into_dyn())?;
...
#[derive(Debug, Clone)]
pub struct CLIPVisionEncoder<F: Float + Debug + ScalarOperand> {
    /// Vision Transformer
...
impl<F: Float + Debug + ScalarOperand> CLIPVisionEncoder<F> {
    /// Create a new CLIPVisionEncoder
    pub fn new(config: ViTConfig, projection_dim: usize) -> Result<Self> {
        // Create ViT with no classification head
        let vision_transformer = VisionTransformer::<F>::new(ViTConfig {
            include_top: false,
            ..config
        })?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for CLIPVisionEncoder<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
...
#[derive(Debug, Clone)]
pub struct CLIP<F: Float + Debug + ScalarOperand> {
    /// Vision encoder
...
impl<F: Float + Debug + ScalarOperand> CLIP<F> {
    ...
    pub fn forward_contrastive(
        ...
        // Transpose to get logits_per_text
        let logits_per_text = logits_per_image.t().into_dyn();
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for CLIP<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
...
fn normalize_features<F: Float + Debug + ScalarOperand>(
    ...
    let features_2d = features.clone().into_shape((batch_size, feature_dim))?;
    ...
    Ok(normalized.into_shape(shape)?)
}
...
fn compute_similarity<F: Float + Debug + ScalarOperand>(
    ...
    let features_a_2d = features_a.clone().into_shape((batch_a, shape_a[1]))?;
    let features_b_2d = features_b.clone().into_shape((batch_b, shape_b[1]))?;
