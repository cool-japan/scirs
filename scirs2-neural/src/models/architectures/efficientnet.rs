...
struct MBConvBlock<F: Float + Debug + ScalarOperand> {
    /// Block configuration
    config: MBConvConfig,
...
impl<F: Float + Debug + ScalarOperand> MBConvBlock<F> {
    ...
    fn drop_connect<R: rand::Rng>(&self, input: &Array<F, IxDyn>, rng: &mut R) -> Array<F, IxDyn> {
        ...
        if rng.gen::<f64>() > self.drop_connect_rate.to_f64().unwrap() {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for MBConvBlock<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
/// EfficientNet implementation
pub struct EfficientNet<F: Float + Debug + ScalarOperand> {
    /// Model configuration
...
impl<F: Float + Debug + ScalarOperand> EfficientNet<F> {
    ...
    pub fn new(config: EfficientNetConfig) -> Result<Self> {
        ...
        // Final batch normalization
        let head_bn = BatchNorm::new(head_channels, 1e-3, 0.01, true)?;

        // Classifier
        let classifier = Dense::new(head_channels, num_classes, None, true)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for EfficientNet<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        // Check input shape
        let shape = input.shape();
