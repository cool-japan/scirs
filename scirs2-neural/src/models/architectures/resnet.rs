...
struct BasicBlock<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> BasicBlock<F> {
    ...
    pub fn new(
        ...
        let bn1 = BatchNorm::new(out_channels, 1e-5, 0.1, true)?;
        ...
        let bn2 = BatchNorm::new(out_channels, 1e-5, 0.1, true)?;
        ...
        let downsample = if downsample {
            ...
            let ds_bn = BatchNorm::new(out_channels, 1e-5, 0.1, true)?;
            Some((ds_conv, ds_bn))
        } else {
            None
        };
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BasicBlock<F> {
...
struct BottleneckBlock<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> BottleneckBlock<F> {
    ...
    pub fn new(
        ...
        // First batch normalization
        let bn1 = BatchNorm::new(bottleneck_channels, 1e-5, 0.1, true)?;
        ...
        // Second batch normalization
        let bn2 = BatchNorm::new(bottleneck_channels, 1e-5, 0.1, true)?;
        ...
        let bn3 = BatchNorm::new(out_channels, 1e-5, 0.1, true)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for BottleneckBlock<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        // Save identity for skip connection
        let identity = input.clone();
...
/// ResNet implementation
pub struct ResNet<F: Float + Debug + ScalarOperand> {
    ...
    avg_pool: Box<dyn Fn(&Array<F, IxDyn>) -> Result<Array<F, IxDyn>>>,
...
impl<F: Float + Debug + ScalarOperand> ResNet<F> {
    ...
    pub fn new(config: ResNetConfig) -> Result<Self> {
        ...
        // Initial batch normalization
        let bn1 = BatchNorm::new(64, 1e-5, 0.1, true)?;
        ...
        for layer_config in &config.layers {
            let mut layer_blocks: Vec<Box<dyn Layer<F>>> = Vec::new();
        ...
        let dropout = if config.dropout_rate > 0.0 {
            let rate = F::from(config.dropout_rate).unwrap();
            Some(Dropout::new(rate))
        } else {
            None
        };
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for ResNet<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        // Initial convolutional layer
        let mut x = self.conv1.forward(input)?;
        x = self.bn1.forward(&x)?;
...
struct ResNetLayerGroup<F: Float + Debug + ScalarOperand> {
    /// Blocks in this layer
    blocks: Vec<Box<dyn Layer<F>>>,
}

impl<F: Float + Debug + ScalarOperand> Layer<F> for ResNetLayerGroup<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
