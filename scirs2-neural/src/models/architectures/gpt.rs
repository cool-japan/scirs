...
use crate::layers::{
    Dense, Dropout, Embedding, EmbeddingConfig, Layer, LayerNorm,
};
...
struct GPTEmbeddings<F: Float + Debug + ScalarOperand> {
    /// Token embeddings
...
impl<F: Float + Debug + ScalarOperand> GPTEmbeddings<F> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for GPTEmbeddings<F> {
...
struct GPTAttention<F: Float + Debug + ScalarOperand> {
    /// Number of attention heads
    num_attention_heads: usize,
...
impl<F: Float + Debug + ScalarOperand> GPTAttention<F> {
    ...
    pub fn new(config: &GPTConfig) -> Result<Self> {
        ...
        // Projections
        let query = Dense::new(hidden_size, hidden_size, None, true)?;
        let key = Dense::new(hidden_size, hidden_size, None, true)?;
        let value = Dense::new(hidden_size, hidden_size, None, true)?;
        let output = Dense::new(hidden_size, hidden_size, None, true)?;

        // Dropouts
        let attn_dropout_prob = F::from(config.attention_probs_dropout_prob).unwrap();
        let resid_dropout_prob = F::from(config.hidden_dropout_prob).unwrap();
        let attn_dropout = Dropout::new(attn_dropout_prob);
        let resid_dropout = Dropout::new(resid_dropout_prob);
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for GPTAttention<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
struct GPTMLP<F: Float + Debug + ScalarOperand> {
    /// First dense layer
    fc1: Dense<F>,
    /// Second dense layer
    fc2: Dense<F>,
...
impl<F: Float + Debug + ScalarOperand> GPTMLP<F> {
    /// Create GPT MLP
    pub fn new(config: &GPTConfig) -> Result<Self> {
        // Dense layers
        let fc1 = Dense::new(config.hidden_size, config.intermediate_size, None, true)?;

        let fc2 = Dense::new(config.intermediate_size, config.hidden_size, None, true)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for GPTMLP<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
struct GPTBlock<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> GPTBlock<F> {
    /// Create GPT block
    pub fn new(config: &GPTConfig) -> Result<Self> {
        // Layer normalizations
        let layer_norm_eps = F::from(config.layer_norm_eps).unwrap();
        let ln_1 = LayerNorm::new(config.hidden_size, layer_norm_eps)?;
        let ln_2 = LayerNorm::new(config.hidden_size, layer_norm_eps)?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for GPTBlock<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
...
pub struct GPTModel<F: Float + Debug + ScalarOperand> {
    /// Embeddings layer
...
impl<F: Float + Debug + ScalarOperand> GPTModel<F> {
    ...
    pub fn new(config: GPTConfig) -> Result<Self> {
        ...
        // Final layer normalization
        let layer_norm_eps = F::from(config.layer_norm_eps).unwrap();
        let ln_f = LayerNorm::new(config.hidden_size, layer_norm_eps)?;
    ...
    pub fn logits(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        let linear = Dense::new(
            ...
            None,
            false, // No bias for tied weights
        )?;
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for GPTModel<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        // Get embeddings
