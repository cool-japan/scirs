...
use crate::layers::{
    AttentionConfig, Layer, LayerNorm, ParamLayer, SelfAttention,
};
...
pub struct FeedForward<F: Float + Debug> {
...
impl<F: Float + Debug + ScalarOperand + 'static> FeedForward<F> {
    ...
    pub fn new<R: Rng>(d_model: usize, d_ff: usize, dropout: f64, rng: &mut R) -> Result<Self> {
        ...
        let mut create_weight_matrix = |input_size: usize,
                                    output_size: usize,
                                    scale: F|
        ...
        let w1 = create_weight_matrix(d_model, d_ff, scale1)?;
        let w2 = create_weight_matrix(d_ff, d_model, scale2)?;
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for FeedForward<F> {
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        let reshaped_input = input
            .clone()
            .into_shape((batch_size, self.d_model))
        ...
        self.hidden_cache.replace(Some(hidden.clone()));
    ...
    fn backward(&self, input: &Array<F, IxDyn>, grad_output: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        let mut grad_input = Array::zeros(input.dim());
...
impl<F: Float + Debug + ScalarOperand + 'static> ParamLayer<F> for FeedForward<F> {
...
pub struct TransformerEncoderLayer<F: Float + Debug> {
...
impl<F: Float + Debug + ScalarOperand + 'static> TransformerEncoderLayer<F> {
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for TransformerEncoderLayer<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        for idx in ndarray::indices(input.shape()) {
            let idx_arr = idx.as_array_view();
            let idx_slice: Vec<usize> = idx_arr.iter().copied().collect();

            // Add the attention output to the input for the residual connection
            attn_output_residual[&idx_slice] = input[&idx_slice] + attn_output[&idx_slice];
        }
        ...
        for idx in ndarray::indices(norm1_output.shape()) {
            let idx_arr = idx.as_array_view();
            let idx_slice: Vec<usize> = idx_arr.iter().copied().collect();

            // Add the feed-forward output to the normalized attention output for the residual connection
            output[&idx_slice] = norm1_output[&idx_slice] + ff_output[&idx_slice];
        }
...
pub struct TransformerEncoder<F: Float + Debug> {
...
impl<F: Float + Debug + ScalarOperand + 'static> TransformerEncoder<F> {
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for TransformerEncoder<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
