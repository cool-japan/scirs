...
use crate::layers::{
    AttentionConfig, Layer, LayerNorm, MultiHeadAttention, SelfAttention,
};
...
pub struct TransformerDecoderLayer<F: Float + Debug> {
...
impl<F: Float + Debug + ScalarOperand + 'static> TransformerDecoderLayer<F> {
    ...
    pub fn forward_with_encoder(
        ...
        for idx in ndarray::indices(input.shape()) {
            ...
            self_attn_output_residual[&idx_slice] =
                input[&idx_slice] + self_attn_output[&idx_slice];
        }
        ...
        for idx in ndarray::indices(norm1_output.shape()) {
            ...
            // Add the cross-attention output to the normalized self-attention output
            cross_attn_output_residual[&idx_slice] =
                norm1_output[&idx_slice] + cross_attn_output[&idx_slice];
        }
        ...
        for idx in ndarray::indices(norm2_output.shape()) {
            let idx_arr = idx.as_array_view();
            let idx_slice: Vec<usize> = idx_arr.iter().copied().collect();

            // Add the feed-forward output to the normalized cross-attention output
            output[&idx_slice] = norm2_output[&idx_slice] + ff_output[&idx_slice];
        }
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for TransformerDecoderLayer<F> {
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        for idx in ndarray::indices(norm1_output.shape()) {
            ...
            // Add the feed-forward output to the normalized self-attention output
            output_residual[&idx_slice] = norm1_output[&idx_slice] + output[&idx_slice];
        }
...
pub struct TransformerDecoder<F: Float + Debug> {
...
impl<F: Float + Debug + ScalarOperand + 'static> TransformerDecoder<F> {
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for TransformerDecoder<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
...
mod tests {
    ...
    fn test_decoder_causal_attention() {
        ...
        // Forward pass with encoder output
        let output = dec_layer
            .forward_with_encoder(&decoder_input.into_dyn(), &encoder_output)
            .unwrap();
