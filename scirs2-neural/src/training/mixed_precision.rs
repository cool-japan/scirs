...
impl<F: Float + Debug + ScalarOperand, LF: Float + Debug + ScalarOperand>
    MixedPrecisionManager<F, LF>
{
    ...
    pub fn new(config: MixedPrecisionConfig) -> Self {
        Self {
            config,
            loss_scale: F::from(config.initial_loss_scale).unwrap(),
            success_counter: 0,
    ...
    pub fn to_low_precision<L: Layer<F>, L2: Layer<LF>>(
        &self,
    ...
    pub fn train_step<LM: Layer<LF>, HM: Layer<F>, O: Optimizer<F>>(
        &mut self,
        ...
        let low_targets = convert_array::<F, LF>(targets)?;
    ...
    pub fn train_epoch<LM: Layer<LF>, HM: Layer<F>, O: Optimizer<F>>(
        &mut self,
        low_precision_model: &mut LM,
        high_precision_model: &mut HM,
        optimizer: &mut O,
...
pub struct MixedPrecisionModel<F: Float + Debug + ScalarOperand, LF: Float + Debug + ScalarOperand>
{
    ...
    MixedPrecisionModel<F, LF>
{
    ...
    pub fn new<HM, LM>(
        ...
        low_precision_model: LM,
        config: MixedPrecisionConfig,
    ) -> Result<Self>
    where
        HM: Layer<F> + Send + Sync + 'static,
    ...
    /// Train for one epoch with mixed precision
    pub fn train_epoch<O: Optimizer<F>>(
        &mut self,
        optimizer: &mut O,
        dataset: &dyn Dataset<F>,
        loss_fn: &dyn Loss<F>,
...
impl<F: Float + Debug + ScalarOperand, LF: Float + Debug + ScalarOperand, O: Optimizer<F>>
