...
/// Gradient statistics
#[derive(Debug, Clone)]
pub struct GradientStats<F: Float + Debug + ScalarOperand> {
...
#[derive(Debug)]
pub struct GradientAccumulator<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> GradientAccumulator<F> {
    ...
    pub fn accumulate_gradients<L: Layer<F>>(
        &mut self,
        model: &mut L,
    ...
    pub fn apply_gradients<L: Layer<F>, O: Optimizer<F>>(
        &mut self,
        model: &mut L,
        optimizer: &mut O,
    ) -> Result<()> {
...
pub struct AccumulationTrainingStep<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> AccumulationTrainingStep<F> {
    ...
    pub fn train_epoch<L: Layer<F>, O: Optimizer<F>>(
        ...
        // Create data loader
        let data_loader = DataLoader::new(dataset, batch_size, shuffle, 0);
        ...
        // Train on batches
        for (batch_idx, (inputs, targets)) in data_loader.take(total_steps).enumerate() {
            // Accumulate gradients
            let loss = self
                .accumulator
            ...
            if self.accumulator.should_update() || batch_idx == total_steps - 1 {
...
#[derive(Debug)]
pub struct GradientAccumulationCallback<F: Float + Debug + ScalarOperand> {
    /// Gradient accumulator
    pub accumulator: GradientAccumulator<F>,
    /// Current epoch
    current_epoch: usize,
...
impl<F: Float + Debug + ScalarOperand> GradientAccumulationCallback<F> {
...
pub struct GradientAccumulationTrainer<F: Float + Debug + ScalarOperand> {
    /// Configuration for gradient accumulation
    pub config: GradientAccumulationConfig,
    /// Model to train
...
impl<F: Float + Debug + ScalarOperand> GradientAccumulationTrainer<F> {
        ...
        LF: Loss<F> + Send + Sync + 'static,
    {
        Self {
            ...
            accumulator: GradientAccumulator::new(config.clone()),
            lr_scheduler: None,
            batch_size,
            shuffle,
            verbose,
    ...
    pub fn train(
        ...
        for epoch in 0..epochs {
            ...
            // Create data loader
            let data_loader = DataLoader::new(train_dataset, self.batch_size, self.shuffle, 0);
            ...
            for (batch_idx, (inputs, targets)) in data_loader.enumerate() {
                ...
                // If it's time to update parameters or it's the last batch
                if self.accumulator.should_update() || batch_idx == total_batches - 1 {
                    // Apply accumulated gradients
                    self.accumulator
    ...
    pub fn evaluate(&self, dataset: &dyn Dataset<F>) -> Result<F> {
        // Create data loader
        let data_loader = DataLoader::new(dataset, self.batch_size, false, 0);
