...
#[derive(Debug, Clone)]
pub struct LossMetric<F: Float + Debug + ScalarOperand> {
    /// Total loss
    total_loss: F,
    /// Number of batches
    num_batches: usize,
...
impl<F: Float + Debug + ScalarOperand> LossMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for LossMetric<F> {
    fn update(
        &mut self,
...
#[derive(Debug, Clone)]
pub struct AccuracyMetric<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> AccuracyMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for AccuracyMetric<F> {
    fn update(
        &mut self,
        ...
        let preds_2d = if preds.ndim() > 2 {
            ...
            preds.into_shape((batch_size, total_classes)).unwrap()
        } else if preds.ndim() == 1 {
            // Binary classification with single output
            preds.into_shape((preds.len(), 1)).unwrap()
        } else {
        ...
        let targets_2d = if targets.ndim() > 2 {
            ...
            targets.into_shape((batch_size, total_classes)).unwrap()
        } else if targets.ndim() == 1 {
            // Binary classification with single output
            targets.into_shape((targets.len(), 1)).unwrap()
        } else {
...
#[derive(Debug, Clone)]
pub struct PrecisionMetric<F: Float + Debug + ScalarOperand> {
    /// True positives
    tp: usize,
    /// False positives
    fp: usize,
...
impl<F: Float + Debug + ScalarOperand> PrecisionMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for PrecisionMetric<F> {
    fn update(
        &mut self,
        predictions: &Array<F, IxDyn>,
        targets: &Array<F, IxDyn>,
        ...
        if predictions.shape()[predictions.ndim() - 1] == 1 || predictions.ndim() == 1 {
            ...
            let targets = targets
                .clone()
                .into_dimensionality::<Ix2>()
                .unwrap_or_else(|_| targets.clone().into_shape((targets.len(), 1)).unwrap());
...
#[derive(Debug, Clone)]
pub struct RecallMetric<F: Float + Debug + ScalarOperand> {
    /// True positives
    tp: usize,
    /// False negatives
    fn_: usize,
...
impl<F: Float + Debug + ScalarOperand> RecallMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for RecallMetric<F> {
    fn update(
        &mut self,
        predictions: &Array<F, IxDyn>,
        targets: &Array<F, IxDyn>,
...
#[derive(Debug, Clone)]
pub struct F1ScoreMetric<F: Float + Debug + ScalarOperand> {
    /// Precision metric
...
impl<F: Float + Debug + ScalarOperand> F1ScoreMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for F1ScoreMetric<F> {
    fn update(
        &mut self,
        predictions: &Array<F, IxDyn>,
        targets: &Array<F, IxDyn>,
...
#[derive(Debug, Clone)]
pub struct MeanSquaredErrorMetric<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> MeanSquaredErrorMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for MeanSquaredErrorMetric<F> {
    fn update(
        &mut self,
        predictions: &Array<F, IxDyn>,
        targets: &Array<F, IxDyn>,
        ...
        // Flatten arrays
        let preds_flat = predictions.clone().into_shape(predictions.len()).unwrap();
        let targets_flat = targets.clone().into_shape(targets.len()).unwrap();
...
#[derive(Debug, Clone)]
pub struct MeanAbsoluteErrorMetric<F: Float + Debug + ScalarOperand> {
...
impl<F: Float + Debug + ScalarOperand> MeanAbsoluteErrorMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for MeanAbsoluteErrorMetric<F> {
    fn update(
        &mut self,
        predictions: &Array<F, IxDyn>,
        targets: &Array<F, IxDyn>,
...
#[derive(Debug, Clone)]
pub struct RSquaredMetric<F: Float + Debug + ScalarOperand> {
    /// Sum of squared errors
    sum_squared_error: F,
...
impl<F: Float + Debug + ScalarOperand> RSquaredMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for RSquaredMetric<F> {
    fn update(
        &mut self,
        predictions: &Array<F, IxDyn>,
        targets: &Array<F, IxDyn>,
...
#[derive(Debug, Clone)]
pub struct AUCMetric<F: Float + Debug + ScalarOperand> {
    /// Prediction scores
    scores: Vec<F>,
    /// True labels
    labels: Vec<F>,
...
impl<F: Float + Debug + ScalarOperand> AUCMetric<F> {
...
impl<F: Float + Debug + ScalarOperand> Metric<F> for AUCMetric<F> {
    fn update(
        &mut self,
        predictions: &Array<F, IxDyn>,
        targets: &Array<F, IxDyn>,
