...
#[derive(Debug)]
pub struct ValidationHandler<F: Float + Debug + ScalarOperand> {
    /// Configuration for validation
...
/// Early stopping state
#[derive(Debug)]
struct EarlyStoppingState<F: Float + Debug + ScalarOperand> {
    /// Configuration for early stopping
...
impl<F: Float + Debug + ScalarOperand> ValidationHandler<F> {
