...
#[derive(Debug)]
pub struct CrossValidator<F: Float + Debug + ScalarOperand> {
    /// Configuration for cross-validation
...
impl<F: Float + Debug + ScalarOperand> CrossValidator<F> {
    ...
    pub fn create_folds(&self, dataset: &dyn Dataset<F>) -> Result<Vec<CrossValidationFold>> {
        ...
        match self.config.strategy {
            CrossValidationStrategy::KFold(k) => {
                if k < 2 {
                    return Err(Error::ValidationError("k must be at least 2".to_string()));
                }
    ...
    pub fn cross_validate<L: Layer<F> + Clone>(
        ...
        for (fold_idx, fold) in folds.iter().enumerate() {
            ...
            struct DatasetView<'a, F: Float + Debug + ScalarOperand> {
                dataset: &'a dyn Dataset<F>,
                indices: &'a [usize],
            }
            
            impl<'a, F: Float + Debug + ScalarOperand> DatasetView<'a, F> {
                fn new(dataset: &'a dyn Dataset<F>, indices: &'a [usize]) -> Self {
                    Self { dataset, indices }
                }
            }
            
            impl<'a, F: Float + Debug + ScalarOperand> Dataset<F> for DatasetView<'a, F> {
                fn len(&self) -> usize {
                    self.indices.len()
                }
                
                fn get(&self, idx: usize) -> Result<(ndarray::Array<F, ndarray::IxDyn>, ndarray::Array<F, ndarray::IxDyn>)> {
                    if idx >= self.indices.len() {
                        return Err(crate::error::NeuralError::InferenceError(
                            format!("Index out of bounds: {} >= {}", idx, self.indices.len())
                        ));
                    }
            ...
            let train_dataset = DatasetView::new(dataset, &fold.train_indices);
