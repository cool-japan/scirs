...
#[derive(Debug, Clone)]
pub struct PredictionOutput<F: Float + Debug + ScalarOperand> {
    /// Model predictions
...
#[derive(Debug)]
pub struct TestEvaluator<F: Float + Debug + ScalarOperand> {
    /// Configuration for test set evaluation
...
impl<F: Float + Debug + ScalarOperand> TestEvaluator<F> {
    ...
    pub fn generate_predictions<L: Layer<F>>(
        ...
        let classes = if first_pred.ndim() > 1 && first_pred.shape()[1] > 1 {
            ...
            let mut class_indices = Array::<F, _>::zeros((combined_preds.shape()[0], 1));
        ...
        let probabilities = if first_pred.ndim() > 1 && first_pred.shape()[1] > 1 {
            ...
            let mut probs = Array::<F, _>::zeros((combined_preds.shape()[0], 2));
