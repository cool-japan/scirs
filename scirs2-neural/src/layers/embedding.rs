...
use ndarray::{Array, ArrayBase, Data, Dimension, IxDyn, ScalarOperand};
...
impl<F: Float + Debug + ScalarOperand> Embedding<F> {
    ...
    fn apply_max_norm(&mut self) -> Result<()> {
        if let Some(max_norm) = self.config.max_norm {
            ...
            let p = F::from(norm_type)
                .ok_or_else(|| Error::InvalidValue(format!("Invalid norm_type: {}", norm_type)))?;
            let max_norm = F::from(max_norm)
                .ok_or_else(|| Error::InvalidValue(format!("Invalid max_norm: {}", max_norm)))?;
    ...
    fn forward_impl<D: Dimension>(
        ...
        // Lookup embeddings
        let indices_flat = indices.view().into_shape(indices.len()).unwrap();
...
impl<F: Float + Debug + ScalarOperand> PositionalEmbedding<F> {
    ...
    pub fn new(max_seq_length: usize, embedding_dim: usize, learned: bool) -> Result<Self> {
        ...
        if learned {
            ...
            let weight = Some(initializers::xavier_uniform::<F>(weight_shape)?);
...
pub struct PatchEmbedding<F: Float + Debug + ScalarOperand> {
    ...
    bias_grad: Option<Array<F, IxDyn>>,
    /// Input cache for backpropagation
    input_cache: Option<Array<F, IxDyn>>,
}

impl<F: Float + Debug + ScalarOperand> PatchEmbedding<F> {
...
impl<F: Float + Debug + ScalarOperand> Layer<F> for PatchEmbedding<F> {
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        // Extract patches
        ...
        mutable_self.input_cache = Some(patches.clone());
    ...
    fn backward(&self, input: &Array<F, IxDyn>, grad_output: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        let patches = self.input_cache.as_ref().unwrap();
        let batch_size = patches.shape()[0];
        let num_patches = patches.shape()[1];
        ...
        for e in 0..self.embedding_dim {
            for i in 0..patch_dim {
                mutable_self.weight_grad[[e, i]] =
                    mutable_self.weight_grad[[e, i]] + weight_grad[[e, i]];
            }
        }

        if let (Some(ref mut bg_acc), Some(ref bg)) = (&mut mutable_self.bias_grad, &bias_grad) {
            for e in 0..self.embedding_dim {
    ...
    fn update(&mut self, learning_rate: F) -> Result<()> {
        ...
        for e in 0..self.embedding_dim {
            for i in 0..self.weight.shape()[1] {
                self.weight[[e, i]] = self.weight[[e, i]] - lr * self.weight_grad[[e, i]];
            }
        }
...
mod tests {
    ...
    fn test_patch_embedding() {
        ...
        let input = Array::from_shape_fn(IxDyn(&[2, 3, 32, 32]), |_| rand_gen.gen::<f32>());
