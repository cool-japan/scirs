...
/// ```
pub struct Conv2D<F: Float + Debug> {
...
impl<F: Float + Debug + ScalarOperand + 'static> Conv2D<F> {
    ...
    fn compute_output_shape(&self, input_shape: &[usize]) -> Result<Vec<usize>> {
        ...
        let output_height = (input_height + 2 * self.padding_values.0
            - self.dilation.0 * (self.kernel_size.0 - 1)
            - 1)
            / self.stride.0
            + 1;
        let output_width = (input_width + 2 * self.padding_values.1
                    ...
                    for kw in 0..self.kernel_size.1 {
                        ...
                        for h in 0..output_height {
                            for w in 0..output_width {
                                let input_h = h * self.stride.0 + kh * self.dilation.0
                                    - self.padding_values.0;
                                let input_w = w * self.stride.1 + kw * self.dilation.1
                                    - self.padding_values.1;

                                let value = if input_h < height
                                    && input_w < width
                                    && input_h >= 0
                                    && input_w >= 0
                                {
                    ...
                    for kw in 0..self.kernel_size.1 {
                        ...
                        for h in 0..output_h {
                            for w in 0..output_w {
                                ...
                                if out_h < height && out_w < width && out_h >= 0 && out_w >= 0 {
    ...
    fn convolution_forward(&self, input: &ArrayView<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        self.columns_cache.replace(Some(columns.clone()));
        ...
        for b in 0..batch_size {
            ...
            let output_batch_reshaped = output_batch
                .into_shape((self.out_channels, output_height, output_width))
                .map_err(|e| {
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for Conv2D<F> {
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        if input_shape.len() == 3 {
            ...
            return output
                .into_shape((out_channels, out_height, out_width))
                .map_err(|e| {
    ...
    fn backward(&self, input: &Array<F, IxDyn>, grad_output: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        let columns_ref = self.columns_cache.borrow();
        ...
        let columns = columns_ref.as_ref().unwrap();
...
impl<F: Float + Debug + ScalarOperand + 'static> ParamLayer<F> for Conv2D<F> {
...
pub struct MaxPool2D<F: Float + Debug> {
    ...
    max_indices: RefCell<Option<Array<usize, IxDyn>>>,
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for MaxPool2D<F> {
    ...
    fn backward(&self, input: &Array<F, IxDyn>, grad_output: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        let cached_input = input_ref.as_ref().unwrap();
        let indices = indices_ref.as_ref().unwrap();
