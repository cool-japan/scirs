...
impl<F: Float + Debug + ScalarOperand + 'static> RNN<F> {
    ...
    pub fn new<R: Rng>(
        ...
        let weight_ih_vec: Vec<F> = (0..(hidden_size * input_size))
            .map(|_| {
                ...
                val.map(|v| v * scale_ih).unwrap_or_else(|_| F::zero())
            })
        ...
        let weight_hh_vec: Vec<F> = (0..(hidden_size * hidden_size))
            .map(|_| {
                let val = F::from(rng.random_range(-1.0..1.0)).ok_or_else(|| {
                    NeuralError::InvalidArchitecture("Failed to convert random value".to_string())
                });
                val.map(|v| v * scale_hh).unwrap_or_else(|_| F::zero())
            })
            .collect::<Result<Vec<F>>>()?;
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for RNN<F> {
    fn as_any(&self) -> &dyn std::any::Any {
        self
    }
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        self.hidden_states_cache
            .replace(Some(all_hidden_states.clone()));
    ...
    fn backward(&self, input: &Array<F, IxDyn>, grad_output: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        let hidden_states = hidden_states_ref.as_ref().unwrap();
...
impl<F: Float + Debug + ScalarOperand + 'static> ParamLayer<F> for RNN<F> {
...
impl<F: Float + Debug + ScalarOperand + 'static> LSTM<F> {
    ...
    pub fn new<R: Rng>(input_size: usize, hidden_size: usize, rng: &mut R) -> Result<Self> {
        ...
        let create_weight_matrix = |rows: usize,
                                    cols: usize,
                                    scale: F|
         -> Result<Array<F, IxDyn>> {
            let weights_vec: Vec<F> = (0..(rows * cols))
                .map(|_| {
                    let val = F::from(rng.random_range(-1.0..1.0)).ok_or_else(|| {
        ...
        let bias_ii = Array::zeros(IxDyn(&[hidden_size]));
        let bias_hi = Array::zeros(IxDyn(&[hidden_size]));
        ...
        let bias_if = Array::zeros(IxDyn(&[hidden_size]));
        let bias_hf = Array::zeros(IxDyn(&[hidden_size]));

        // Initialize forget gate biases to 1.0 (common practice to help training)
        let mut bias_if = Array::zeros(IxDyn(&[hidden_size]));
        let mut bias_hf = Array::zeros(IxDyn(&[hidden_size]));
        let one = F::one();
        ...
        let bias_ig = Array::zeros(IxDyn(&[hidden_size]));
        let bias_hg = Array::zeros(IxDyn(&[hidden_size]));
        ...
        let bias_io = Array::zeros(IxDyn(&[hidden_size]));
        let bias_ho = Array::zeros(IxDyn(&[hidden_size]));
    ...
    fn step(
        ...
        // Initialize gates
        let mut i_gate = Array::zeros((batch_size, self.hidden_size));
        let mut f_gate = Array::zeros((batch_size, self.hidden_size));
        let mut g_gate = Array::zeros((batch_size, self.hidden_size));
        let mut o_gate = Array::zeros((batch_size, self.hidden_size));
        ...
        Ok((new_h, new_c, (i_gate, f_gate, g_gate, o_gate)))
    }
}
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for LSTM<F> {
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        for t in 0..seq_len {
            ...
            // Process one step
            let (new_h, new_c, gates) = self.step(&x_t.view(), &h.view(), &c.view())?;
            h = new_h;
            c = new_c;
        ...
        self.hidden_states_cache
            .replace(Some(all_hidden_states.clone()));
        self.cell_states_cache.replace(Some(all_cell_states));
...
impl<F: Float + Debug + ScalarOperand + 'static> GRU<F> {
    ...
    pub fn new<R: Rng>(input_size: usize, hidden_size: usize, rng: &mut R) -> Result<Self> {
        ...
        let bias_ir = Array::zeros(IxDyn(&[hidden_size]));
        let bias_hr = Array::zeros(IxDyn(&[hidden_size]));
        ...
        let bias_iz = Array::zeros(IxDyn(&[hidden_size]));
        let bias_hz = Array::zeros(IxDyn(&[hidden_size]));
        ...
        let bias_in = Array::zeros(IxDyn(&[hidden_size]));
        let bias_hn = Array::zeros(IxDyn(&[hidden_size]));
    ...
    fn step(
        ...
        // Initialize gates
        let mut r_gate = Array::zeros((batch_size, self.hidden_size));
        let mut z_gate = Array::zeros((batch_size, self.hidden_size));
        let mut n_gate = Array::zeros((batch_size, self.hidden_size));
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for GRU<F> {
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        for t in 0..seq_len {
            ...
            // Process one step
            let (new_h, gates) = self.step(&x_t.view(), &h.view())?;
            h = new_h;
            all_gates.push(gates);
