...
use ndarray::{
    Array, ArrayView, IxDyn, ScalarOperand,
};
...
impl<F: Float + Debug + ScalarOperand + 'static> MultiHeadAttention<F> {
    ...
    pub fn new<R: Rng>(d_model: usize, config: AttentionConfig, rng: &mut R) -> Result<Self> {
        ...
        let create_weight_matrix = |size: usize| -> Result<Array<F, IxDyn>> {
    ...
    fn reshape_for_multihead(
        ...
        let reshaped = input
            .to_owned()
            .into_shape(IxDyn(&[batch_size, seq_len, num_heads, head_dim]))
            .map_err(|e| {
    ...
    fn scaled_dot_product_attention(
        ...
        let key_transposed = key.to_owned().permuted_axes([0, 2, 3, 1]).into_dyn();
...
impl<F: Float + Debug + ScalarOperand + 'static> Layer<F> for MultiHeadAttention<F> {
    ...
    fn forward(&self, input: &Array<F, IxDyn>) -> Result<Array<F, IxDyn>> {
        ...
        let input_view = if input.ndim() > 3 {
            ...
            input
                .clone()
                .into_shape(IxDyn(&[flat_batch_size, seq_len, features]))
                .map_err(|e| {
        ...
        let attn_output_reshaped = attn_output
            .into_shape(IxDyn(&[batch_size, seq_len, self.d_model]))
            .map_err(|e| {
        ...
        if input.ndim() > 3 {
            ...
            output.into_shape(IxDyn(&output_shape)).map_err(|e| {
...
mod tests {
    ...
    fn test_causal_attention() {
        ...
        // Forward pass
        let output = mha.forward(&input.into_dyn()).unwrap();
