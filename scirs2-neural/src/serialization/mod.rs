...
/// Save model to file
pub fn save_model<F: Float + Debug + ScalarOperand + 'static, P: AsRef<Path>>(
    model: &Sequential<F>,
    path: P,
...
pub fn load_model<F: Float + Debug + ScalarOperand + 'static, P: AsRef<Path>>(
    path: P,
    format: SerializationFormat,
) -> Result<Sequential<F>> {
...
fn serialize_model<F: Float + Debug + ScalarOperand + 'static>(
    model: &Sequential<F>,
    ...
    for layer in model.layers() {
        if let Some(dense) = layer.as_any().downcast_ref::<Dense<F>>() {
            ...
            // Extract parameters
            let params = extract_parameters(dense.parameters())?;
            ...
            // Extract parameters
            let params = extract_parameters(conv.parameters())?;
            parameters.push(params);
            ...
            // Extract parameters
            let params = extract_parameters(ln.parameters())?;
            parameters.push(params);
            ...
            // Extract parameters
            let params = extract_parameters(bn.parameters())?;
            parameters.push(params);
...
fn extract_parameters<F: Float + Debug + ScalarOperand>(
    ...
    for param in params {
        let f64_vec: Vec<f64> = param
            .iter()
            .map(|&x| x.to_f64().ok_or_else(|| NeuralError::SerializationError("Cannot convert parameter to f64".to_string())))
...
fn deserialize_model<F: Float + Debug + ScalarOperand + 'static>(
...
fn create_dense_layer<F: Float + Debug + ScalarOperand + 'static>(
    config: &DenseConfig,
    params: &[Vec<f64>],
) -> Result<Dense<F>> {
...
fn create_conv2d_layer<F: Float + Debug + ScalarOperand + 'static>(
    config: &Conv2DConfig,
    params: &[Vec<f64>],
) -> Result<Conv2D<F>> {
    let mut rng = rand::rng();
...
fn create_layer_norm<F: Float + Debug + ScalarOperand + 'static>(
...
fn create_batch_norm<F: Float + Debug + ScalarOperand + 'static>(
...
fn create_dropout<F: Float + Debug + ScalarOperand + 'static>(
...
fn create_maxpool2d<F: Float + Debug + ScalarOperand + 'static>(
...
fn array_from_vec<F: Float + Debug + ScalarOperand + 'static>(
    vec: &[f64],
    shape: &[usize],
...
impl ActivationFunction {
    ...
    pub fn create<F: Float + Debug + ScalarOperand>(&self) -> Box<dyn Activation<F>> {
        match self {
            ActivationFunction::ReLU => Box::new(ReLU::<F>::new()),
            ActivationFunction::Sigmoid => Box::new(Sigmoid::<F>::new()),
            ActivationFunction::Tanh => Box::new(Tanh::<F>::new()),
            ActivationFunction::Softmax => Box::new(Softmax::<F>::new()),
            ActivationFunction::LeakyReLU(alpha) => {
                Box::new(LeakyReLU::<F>::new(F::from(*alpha).unwrap()))
            }
            ActivationFunction::ELU(alpha) => Box::new(ELU::<F>::new(F::from(*alpha).unwrap())),
            ActivationFunction::GELU => Box::new(GELU::<F>::new()),
            ActivationFunction::Swish => Box::new(Swish::<F>::new()),
            ActivationFunction::Mish => Box::new(Mish::<F>::new()),
        }
    }
}
...
impl ActivationFactory {
    ...
    pub fn create<F: Float + Debug + ScalarOperand>(name: &str) -> Option<Box<dyn Activation<F>>> {
