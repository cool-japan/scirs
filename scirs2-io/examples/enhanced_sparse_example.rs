...
use scirs2_io::serialize::{
    SparseMatrix, SparseMatrixCOO, SparseFormat, SerializationFormat,
    serialize_enhanced_sparse_matrix, deserialize_enhanced_sparse_matrix,
    sparse_ops,
};
...
fn demonstrate_sparse_formats() -> Result<()> {
    ...
    println!("      Dimensions: {}x{}", sparse.shape().0, sparse.shape().1);
    ...
    println!("    Memory usage after conversions: {} bytes", sparse.memory_usage());
...
fn demonstrate_sparse_operations() -> Result<()> {
    ...
    println!("  Matrix A: {}x{} with {} non-zeros", 
             matrix_a.shape().0, matrix_a.shape().1, matrix_a.nnz());
    println!("  Matrix B: {}x{} with {} non-zeros", 
             matrix_b.shape().0, matrix_b.shape().1, matrix_b.nnz());
    ...
    println!("    Result: {}x{} with {} non-zeros", 
             sum_result.rows, sum_result.cols, sum_result.nnz());
    ...
    println!("    Original: {}x{}, Transposed: {}x{}", 
             matrix_a.shape().0, matrix_a.shape().1,
             transpose_result.rows, transpose_result.cols);
    ...
    println!("    Matrix-vector multiplication completed in {:?}", matvec_time);
...
fn demonstrate_serialization() -> Result<()> {
    ...
    // Add metadata
    sparse.metadata.insert("description".to_string(), "Test tridiagonal matrix".to_string());
    sparse.metadata.insert("created_by".to_string(), "Enhanced sparse example".to_string());
    sparse.metadata.insert("matrix_type".to_string(), "tridiagonal".to_string());
    
    println!("  Original matrix: {}x{} with {} non-zeros", 
             sparse.shape().0, sparse.shape().1, sparse.nnz());
    ...
    for (format, filename) in formats {
        ...
        println!("    Serialize: {:?}, Deserialize: {:?}", serialize_time, deserialize_time);
        println!("    File size: {} bytes", file_size);
        println!("    Loaded matrix: {}x{} with {} non-zeros", 
                 loaded.shape().0, loaded.shape().1, loaded.nnz());
...
fn demonstrate_memory_analysis() -> Result<()> {
    ...
    let coo_memory = sparse.coo_data.values.len() * (std::mem::size_of::<f64>() + 2 * std::mem::size_of::<usize>());
...
mod benchmarks {
    ...
    pub fn benchmark_format_conversions(size: usize, iterations: usize) -> Result<()> {
        println!("Benchmarking format conversions for {}x{} matrix ({} iterations):", size, size, iterations);
        ...
        println!("  Average COO -> CSR: {:?}", total_csr_time / iterations as u32);
        println!("  Average COO -> CSC: {:?}", total_csc_time / iterations as u32);
