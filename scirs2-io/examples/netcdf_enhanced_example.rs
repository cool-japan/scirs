...
use scirs2_io::netcdf::{NetCDFFile, NetCDFDataType, NetCDFOptions};
...
fn create_climate_dataset_structure() -> CoreResult<()> {
    ...
    nc_file.create_variable("temperature", NetCDFDataType::Float, &["time", "lat", "lon"])?;
    nc_file.create_variable("precipitation", NetCDFDataType::Float, &["time", "lat", "lon"])?;
    ...
    for var_name in nc_file.variables() {
        ...
        println!("    Variable '{}': {:?}, dimensions: {:?}", var_name, dtype, dims);
        if !attrs.is_empty() {
...
fn create_scientific_dataset_structure() -> CoreResult<()> {
    ...
    nc_file.create_variable("field_intensity", NetCDFDataType::Double, &["time", "x", "y"])?;
    nc_file.create_variable("sensor_readings", NetCDFDataType::Float, &["time", "sensors"])?;
    ...
    nc_file.add_variable_attribute("field_intensity", "long_name", "electromagnetic field intensity")?;
    nc_file.add_variable_attribute("field_intensity", "description", "Measured field intensity showing wave interference patterns")?;
    ...
    nc_file.add_variable_attribute("experiment_notes", "long_name", "experimental notes and comments")?;
    nc_file.add_variable_attribute("experiment_notes", "description", "Free-form text describing experimental conditions")?;
...
fn demonstrate_metadata_features() -> CoreResult<()> {
    ...
    nc_file.add_global_attribute("summary", "Hourly meteorological measurements from 10 weather stations")?;
    ...
    nc_file.add_global_attribute("references", "Station network described in Smith et al. (2023)")?;
    ...
    nc_file.add_variable_attribute("temperature", "instrument", "Platinum resistance thermometer")?;
