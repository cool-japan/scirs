...
use scirs2_io::image::{
    read_exif_metadata, read_enhanced_metadata, read_image, write_image,
    ...
    match read_exif_metadata("test_image.jpg")? {
        Some(exif) => {
            ...
            if let Some(gps) = &exif.gps {
                ...
                println!("        Latitude: {:.6}° {}", gps.latitude, gps.latitude_ref);
                println!("        Longitude: {:.6}° {}", gps.longitude, gps.longitude_ref);
        ...
        match read_enhanced_metadata(filename) {
            Ok(metadata) => {
                ...
                if let Some(color_mode) = metadata.color_mode {
                    println!("      Color Mode: {} ({} channels)", 
                             color_mode.as_str(), color_mode.channels());
                }
                ...
                if let Some(exif) = &metadata.exif {
                    ...
                    if exif.camera.make.is_some() || exif.camera.model.is_some() {
                        println!("      Camera: {:?} {:?}", exif.camera.make, exif.camera.model);
                    }
                } else {
...
fn analyze_image_formats() -> Result<()> {
    ...
    let formats = [
        ...
        (ImageFormat::JPEG, "JPEG", "Lossy compression, supports EXIF"),
        (ImageFormat::BMP, "BMP", "Uncompressed, simple format"),
        (ImageFormat::TIFF, "TIFF", "Flexible, supports EXIF and multiple pages"),
    ];
    ...
    for (format, name, description) in &formats {
        ...
        println!("    EXIF Support: {}", if exif_support { "Yes" } else { "No" });
...
#[allow(dead_code)]
fn calculate_compression_ratio(width: usize, height: usize, channels: usize, file_size: u64) -> f64 {
