...
use scirs2_io::matrix_market::{
    ...
    MMSparseMatrix, MMDenseMatrix, MMHeader, SparseEntry,
    MMFormat, MMDataType, MMSymmetry,
    sparse_to_coo, coo_to_sparse,
};
...
fn create_sparse_matrix_example() -> Result<()> {
    ...
    entries.push(SparseEntry { row: 0, col: 2, value: 0.5 });
    entries.push(SparseEntry { row: 1, col: 3, value: -1.2 });
    entries.push(SparseEntry { row: 2, col: 4, value: 2.3 });
    entries.push(SparseEntry { row: 3, col: 0, value: -0.8 });
    entries.push(SparseEntry { row: 4, col: 1, value: 1.7 });
...
fn read_and_analyze_matrices() -> Result<()> {
    ...
    println!("      Sparsity: {:.2}% ({} zeros out of {} total elements)", 
             sparsity * 100.0, total_elements - sparse_matrix.nnz, total_elements);
    
    // Show some entries
    println!("      First 5 non-zero entries:");
    for (i, entry) in sparse_matrix.entries.iter().take(5).enumerate() {
        println!("        ({}, {}) = {}", entry.row + 1, entry.col + 1, entry.value);
    }
    ...
    let min = dense_matrix.data.iter().fold(f64::INFINITY, |a, &b| a.min(b));
    let max = dense_matrix.data.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
...
fn demonstrate_matrix_types() -> Result<()> {
    ...
    let sym_entries = vec![
        SparseEntry { row: 0, col: 0, value: 2.0 },   // Diagonal
        SparseEntry { row: 1, col: 0, value: 1.0 },   // Lower triangle only
        SparseEntry { row: 1, col: 1, value: 3.0 },   // Diagonal
        SparseEntry { row: 2, col: 0, value: 0.5 },   // Lower triangle only
        SparseEntry { row: 2, col: 1, value: -0.8 },  // Lower triangle only
        SparseEntry { row: 2, col: 2, value: 4.0 },   // Diagonal
    ];
    ...
    println!("    Written symmetric matrix (3x3) with {} stored entries", sym_matrix.nnz);
    ...
    let pattern_entries = vec![
        SparseEntry { row: 0, col: 0, value: 1.0 },   // Value ignored for pattern
        SparseEntry { row: 0, col: 3, value: 1.0 },
        SparseEntry { row: 1, col: 1, value: 1.0 },
        SparseEntry { row: 1, col: 4, value: 1.0 },
        SparseEntry { row: 2, col: 2, value: 1.0 },
        SparseEntry { row: 3, col: 0, value: 1.0 },
        SparseEntry { row: 3, col: 3, value: 1.0 },
        SparseEntry { row: 4, col: 1, value: 1.0 },
        SparseEntry { row: 4, col: 4, value: 1.0 },
    ];
    ...
    println!("    Written pattern matrix (5x5) with {} non-zero positions", pattern_matrix.nnz);
    ...
    let int_entries = vec![
        SparseEntry { row: 0, col: 0, value: 10.0 },
        SparseEntry { row: 1, col: 1, value: 20.0 },
        SparseEntry { row: 2, col: 2, value: 30.0 },
        SparseEntry { row: 0, col: 2, value: -5.0 },
        SparseEntry { row: 2, col: 0, value: 15.0 },
    ];
    ...
    println!("    Written integer matrix (3x3) with {} entries", int_matrix.nnz);
...
fn demonstrate_coordinate_conversions() -> Result<()> {
    ...
    let values_match = values.iter().zip(back_values.iter())
