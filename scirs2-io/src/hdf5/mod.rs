...
pub enum HDF5DataType {
    ...
    /// Array type
    Array { base_type: Box<HDF5DataType>, shape: Vec<usize> },
    /// Compound type
    Compound { fields: Vec<(String, HDF5DataType)> },
    /// Enum type
...
impl Group {
    ...
    pub fn create_group(&mut self, name: &str) -> &mut Group {
        self.groups.entry(name.to_string())
...
impl HDF5File {
        ...
        D: ndarray::Dimension,
    {
        ...
        let flat_data: Vec<f64> = array.iter()
            .map(|x| x.clone().into())
            .collect();
        ...
        current_group.datasets.insert(dataset_name.to_string(), dataset);
        Ok(())
    }
    ...
    pub fn read_dataset(&self, path: &str) -> Result<ArrayD<f64>> {
        ...
        for &group_name in &parts[..parts.len() - 1] {
            current_group = current_group.get_group(group_name)
                .ok_or_else(|| IoError::FormatError(format!("Group '{}' not found", group_name)))?;
        }
        
        // Get the dataset
        let dataset = current_group.datasets.get(*dataset_name)
        ...
        match &dataset.data {
            ...
            _ => Err(IoError::FormatError("Unsupported data type for ndarray conversion".to_string())),
        }
    }
...
mod tests {
    ...
    fn test_attribute_setting() {
        ...
        group.set_attribute("description", AttributeValue::String("Test group".to_string()));
