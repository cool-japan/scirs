...
impl TSNE {
        ...
        S::Elem: Float + NumCast,
    {
        ...
        if self.method == "barnes_hut" && self.n_components > 3 {
            return Err(TransformError::InvalidInput(
                "'n_components' should be less than or equal to 3 for barnes_hut algorithm".to_string()
            ));
        }
        ...
        let (embedding, kl_divergence, n_iter) = self.tsne_optimization(p, x_embedded, n_samples)?;
    ...
    fn initialize_embedding(&self, x: &Array2<f64>) -> Result<Array2<f64>> {
        ...
        if self.init == "pca" {
            ...
            Err(TransformError::InvalidInput(format!(
                "Initialization method '{}' not recognized", self.init
            )))
        }
    }
    ...
    fn distances_to_affinities(&self, distances: &Array2<f64>) -> Result<Array2<f64>> {
        ...
        for i in 0..n_samples {
            ...
            for _ in 0..50 {  // Usually converges within 50 iterations
    ...
    fn tsne_optimization(
        ...
        for i in 0..exploration_n_iter {
            ...
            let (curr_error, grad) = self.compute_gradient_exact(&embedding, &p_early, degrees_of_freedom)?;
            
            // Perform gradient update with momentum and gains
            self.gradient_update(&mut embedding, &mut update, &mut gains, &grad, 0.5, self.learning_rate_)?;
        ...
        for i in iter+1..self.max_iter {
            ...
            let (curr_error, grad) = self.compute_gradient_exact(&embedding, &p, degrees_of_freedom)?;
            error = curr_error;
            
            // Perform gradient update with momentum and gains
            self.gradient_update(&mut embedding, &mut update, &mut gains, &grad, 0.8, self.learning_rate_)?;
        ...
        if self.verbose {
            println!("[t-SNE] Optimization finished after {} iterations with error {:.7}", iter + 1, error);
        }
    ...
    fn compute_gradient_exact(
        ...
        for i in 0..n_samples {
            for j in i+1..n_samples {
                ...
                let q_ij = (1.0 + d_squared / degrees_of_freedom).powf(-(degrees_of_freedom + 1.0) / 2.0);
                dist[[i, j]] = q_ij;
                dist[[j, i]] = q_ij;
            }
        }
        ...
        for i in 0..n_samples {
            for j in 0..n_samples {
                if i != j {
                    ...
                    for k in 0..n_components {
                        grad[[i, k]] += factor * p_q_diff * dist[[i, j]] * (embedding[[i, k]] - embedding[[j, k]]);
                    }
                }
            }
        }
    ...
    for i in 0..n_samples {
        for &j in nn_embedded.row(i).iter() {
            ...
            if is_not_neighbor {
                ...
                let row = dist_x.row(i).to_owned();
                let mut pairs: Vec<(usize, f64)> = row.iter().enumerate().map(|(idx, &d)| (idx, d)).collect();
                pairs.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal));
