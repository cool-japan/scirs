use scirs2_signal::window::{hamming, hann, bartlett};
...
fn print_window_analysis(window: &[f64], name: &str) {
    ...
    match name {
        "Hamming" => {
            ...
            let center_idx = (n - 1) as f64 / 2.0;
            let theoretical_max = 0.54 + 0.46; // cos(π) = -1
            println!("Theoretical maximum: {} (at symmetric center)", theoretical_max);
            
            // For even N, calculate actual center values
            if n % 2 == 0 {
                let idx1 = n / 2 - 1;
                let idx2 = n / 2;
                let val1 = 0.54 - 0.46 * (2.0 * std::f64::consts::PI * idx1 as f64 / (n - 1) as f64).cos();
                let val2 = 0.54 - 0.46 * (2.0 * std::f64::consts::PI * idx2 as f64 / (n - 1) as f64).cos();
        ...
        "Hann" => {
            ...
            if n % 2 == 0 {
                let idx1 = n / 2 - 1;
                let idx2 = n / 2;
                let val1 = 0.5 * (1.0 - (2.0 * std::f64::consts::PI * idx1 as f64 / (n - 1) as f64).cos());
                let val2 = 0.5 * (1.0 - (2.0 * std::f64::consts::PI * idx2 as f64 / (n - 1) as f64).cos());
                println!("Expected at index {}: {:.6}", idx1, val1);
                println!("Expected at index {}: {:.6}", idx2, val2);
            }
        }
        "Bartlett" => {
            ...
            if n % 2 == 0 {
                ...
                println!("Expected maximum: {:.6} (at indices {} and {})", expected, idx, idx + 1);
            } else {
                let idx = n / 2;
