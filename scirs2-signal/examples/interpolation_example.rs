...
use scirs2_signal::{interpolate, SignalResult};
...
fn generate_test_signal(n_samples: usize, missing_rate: f64) -> Array1<f64> {
    ...
    for i in 0..n_samples {
        if rand::random::<f64>() < missing_rate {
            signal[i] = f64::NAN;
        }
    }
...
fn compare_interpolation_methods() -> SignalResult<()> {
    ...
    for i in 0..n_samples {
        ...
        reference[i] = (x[i]).sin() + 0.5 * (x[i] * 3.0).sin() + 0.2 * (x[i] * 7.0).sin();
...
fn interpolate_bandlimited_signal() -> SignalResult<()> {
    ...
    let mut spectrum = vec![Complex::new(0.0, 0.0); n_samples];
    ...
    for i in 1..cutoff {
        let amplitude = normal.sample(&mut rng).abs();
        let phase = rand::random::<f64>() * 2.0 * PI;

        // Symmetric for real signal
        spectrum[i] = Complex::new(amplitude * phase.cos(), amplitude * phase.sin());
        spectrum[n_samples - i] = Complex::new(amplitude * phase.cos(), -amplitude * phase.sin());
    }

    // DC component
    spectrum[0] = Complex::new(normal.sample(&mut rng).abs(), 0.0);

    // Create Nyquist component if n_samples is even
    if n_samples % 2 == 0 {
        spectrum[n_samples / 2] = Complex::new(normal.sample(&mut rng).abs(), 0.0);
    }
...
fn auto_interpolation_example() -> SignalResult<()> {
    ...
    for i in 0..n_samples {
        if x[i] < 3.0 {
            ...
            // Oscillatory region
            reference[i] = 2.0 + 0.5 * (x[i] - 6.0).sin() * (x[i] - 6.0);
        }
    }
