...
use scirs2_signal::wiener::{
    wiener_filter, wiener_filter_freq, wiener_filter_time,
    iterative_wiener_filter, wiener_filter_2d, spectral_subtraction,
    psd_wiener_filter, kalman_wiener_filter, WienerConfig
};
...
fn main() {
    ...
    save_signals_to_csv(
        "wiener_basic.csv",
        &clean_signal, &noisy_signal, &denoised_basic,
        None
    );
    ...
    save_signals_to_csv(
        ...
        &clean_signal, &noisy_signal, &denoised_freq,
        Some(&denoised_time)
    );
    ...
    save_signals_to_csv(
        ...
        &clean_signal, &noisy_signal, &denoised_spec1,
        Some(&denoised_spec2)
    );
    ...
    save_signals_to_csv(
        "wiener_psd.csv",
        &clean_signal, &noisy_signal, &denoised_psd,
        None
    );
    ...
    let nonstat_kalman = kalman_wiener_filter(
        &noisy_signal2, process_var, measurement_var
    ).unwrap();
    ...
    println!("   Non-stationary signal input SNR: {:.2} dB", nonstat_input_snr);
    println!("   Standard Wiener filter SNR: {:.2} dB", nonstat_wiener_snr);
    println!("   Kalman-Wiener filter SNR: {:.2} dB", nonstat_kalman_snr);
    
    save_signals_to_csv(
        "wiener_nonstationary.csv",
        &clean_signal2, &noisy_signal2, &nonstat_wiener,
        Some(&nonstat_kalman)
    );
    ...
    println!("   Improvement: {:.2} dB", image_output_snr - image_input_snr);
    
    save_image_to_csv(
        "wiener_image_clean.csv", &clean_image
    );
    save_image_to_csv(
        "wiener_image_noisy.csv", &noisy_image
    );
    save_image_to_csv(
        "wiener_image_denoised.csv", &denoised_image
    );
...
fn generate_nonstationary_signal() -> (Array1<f64>, Array1<f64>) {
    ...
    let clean_signal = t.mapv(|ti| {
        (2.0 * PI * f1 * ti).sin() + 0.5 * (2.0 * PI * f2 * ti).sin()
    });
...
fn save_signals_to_csv(
    ...
    // Write data
    for i in 0..clean.len() {
        write!(file, "{},{},{},{}", i, clean[i], noisy[i], denoised1[i]).expect("Failed to write data");
