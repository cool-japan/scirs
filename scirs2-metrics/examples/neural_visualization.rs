...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for epoch in 0..num_epochs {
        ...
        epoch_metrics.insert(
            "loss".to_string(),
            1.0 - 0.8 * progress + rng.gen::<f64>() * 0.1,
        );
        epoch_metrics.insert(
            "accuracy".to_string(),
            0.5 + 0.4 * progress + rng.gen::<f64>() * 0.05,
        );
        ...
        val_epoch_metrics.insert(
            "loss".to_string(),
            1.1 - 0.7 * progress + rng.gen::<f64>() * 0.15,
        );
        val_epoch_metrics.insert(
            "accuracy".to_string(),
            0.45 + 0.35 * progress + rng.gen::<f64>() * 0.08,
        );
        val_history.push(val_epoch_metrics);
    }

    // Create training history visualization
    let training_viz = scirs2_metrics::integration::neural::training_history_visualization(
        metrics.clone(),
        history.clone(),
    ...
    for i in 0..n_samples {
        if y_true[i] > 0.5 {
            ...
            y_score[i] = 0.7 + rng.gen::<f64>() * 0.3;
            // Add some false negatives
            if rng.gen::<f64>() < 0.2 {
                y_score[i] = rng.gen::<f64>() * 0.5;
            }
        } else {
            ...
            if rng.gen::<f64>() < 0.1 {
                y_score[i] = 0.5 + rng.gen::<f64>() * 0.5;
            }
        }
    }

    // Create ROC curve visualization
    let roc_viz = scirs2_metrics::integration::neural::neural_roc_curve_visualization(
    ...
    let pr_viz = scirs2_metrics::integration::neural::neural_precision_recall_curve_visualization(
        &y_true.clone().into_dyn(),
        &y_score.clone().into_dyn(),
    ...
    for i in 0..n_samples {
        ...
        let pred_class = if rng.gen::<f64>() < 0.7 {
            true_class // Correct prediction
        } else {
            (true_class + 1 + (rng.gen::<f64>() * (classes as f64 - 2.0)) as usize) % classes
            // Error
        };
    ...
    let cm_viz = scirs2_metrics::integration::neural::neural_confusion_matrix_visualization(
