...
use ndarray::{self, ArrayBase, Array1, Array2, Data, Dimension};
...
/// use scirs2_metrics::classification::advanced::log_loss;
///
/// // For binary classification
/// let y_true = array![0, 1, 1, 0];
/// let y_prob = array![0.1, 0.9, 0.8, 0.3];
///
/// let loss = log_loss(&y_true.insert_axis(ndarray::Axis(1)), 
///                     &Array2::from_shape_fn((4, 2), |(i, j)| {
///                         if j == 0 { 1.0 - y_prob[i] } else { y_prob[i] }
///                     }), 
///                     1e-15, true).unwrap();
///
...
/// let y_prob = Array2::from_shape_vec((4, 3), vec![
...
/// ]).unwrap();
///
/// let loss = log_loss(&y_true, &y_prob, 1e-15, true).unwrap();
/// ```
    ...
    S2: Data<Elem = f64>,
{
    ...
    for i in 0..n_samples {
        ...
        for j in 0..n_classes {
            let true_val: f64 = if let Some(val) = NumCast::from(y_true.get((i, j))
                .ok_or_else(|| MetricsError::InvalidInput("Index out of bounds accessing y_true".to_string()))?
                .clone()) {
                val
            } else {
                return Err(MetricsError::InvalidInput(
            ...
            if true_val > 0.0 {
                let prob = y_prob.get((i, j))
                    .ok_or_else(|| MetricsError::InvalidInput("Index out of bounds accessing y_prob".to_string()))?
                    .max(eps).min(1.0 - eps);
    ...
    D2: Dimension,
{
    ...
    match (y_true.ndim(), y_prob.ndim()) {
        (1, 1) => {
            ...
            let y_true_1d = y_true.view().into_dimensionality::<ndarray::Ix1>()
                .map_err(|_| MetricsError::InvalidInput("Error converting y_true to 1D".to_string()))?;
            
            let y_prob_1d = y_prob.view().into_dimensionality::<ndarray::Ix1>()
                .map_err(|_| MetricsError::InvalidInput("Error converting y_prob to 1D".to_string()))?;
        ...
        (1, 2) => {
            ...
            let y_prob_2d = y_prob.view().into_dimensionality::<ndarray::Ix2>()
                .map_err(|_| MetricsError::InvalidInput("Error converting y_prob to 2D".to_string()))?;
            ...
            for i in 0..n_samples {
                let class_idx: usize = if let Some(idx) = NumCast::from(y_true_1d[i].clone()) {
                    idx
                } else {
                    return Err(MetricsError::InvalidInput(format!(
                ...
                if let Some(one) = NumCast::from(1) {
                    ...
                    return Err(MetricsError::InvalidInput("Could not cast 1 to element type".to_string()));
                }
            }
        ...
        (2, 2) => {
            ...
            let y_true_2d = y_true.view().into_dimensionality::<ndarray::Ix2>()
                .map_err(|_| MetricsError::InvalidInput("Error converting y_true to 2D".to_string()))?;
        ...
        _ => Err(MetricsError::InvalidInput(
            format!("Unsupported dimensions: y_true ({:?}), y_prob ({:?})", 
    ...
    D2: Dimension,
{
    ...
    for (true_label, prob) in y_true.iter().zip(y_prob.iter()) {
        // Find bin index
        let bin_idx = bin_edges.iter().enumerate()
