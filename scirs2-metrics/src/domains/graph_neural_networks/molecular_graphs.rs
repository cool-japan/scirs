...
impl MolecularGraphMetrics {
    ...
    pub fn evaluate_molecular_properties(
        ...
        for (property, pred_values) in predictions {
            if let Some(true_values) = ground_truth.get(property) {
                ...
                // Calculate MAE
                let mae: f64 = pred_values.iter()
                    .zip(true_values.iter())
                    .map(|(p, t)| (p - t).abs())
                    .sum::<f64>() / pred_values.len() as f64;

                // Calculate RMSE
                let mse: f64 = pred_values.iter()
                    .zip(true_values.iter())
                    .map(|(p, t)| (p - t).powi(2))
                    .sum::<f64>() / pred_values.len() as f64;
                let rmse = mse.sqrt();
                ...
                let ss_res: f64 = pred_values.iter()
                    .zip(true_values.iter())
                    .map(|(p, t)| (t - p).powi(2))
                    .sum();
                ...
                self.property_prediction.property_metrics.insert(property.clone(), property_metrics);
            }
        }

        // Calculate overall metrics
        if !self.property_prediction.property_metrics.is_empty() {
            self.property_prediction.overall_mae = self.property_prediction.property_metrics.values()
                .map(|m| m.mae)
                .sum::<f64>() / self.property_prediction.property_metrics.len() as f64;

            self.property_prediction.overall_r2 = self.property_prediction.property_metrics.values()
                .map(|m| m.r2_score)
                .sum::<f64>() / self.property_prediction.property_metrics.len() as f64;
        }
    ...
    pub fn evaluate_drug_discovery(
        ...
        if admet_predictions.len() == admet_ground_truth.len() && !admet_predictions.is_empty() {
            let correct = admet_predictions.iter()
        ...
        if bioactivity_scores.len() == bioactivity_labels.len() && !bioactivity_scores.is_empty() {
            self.drug_discovery.bioactivity_auc = self.calculate_auc(bioactivity_scores, bioactivity_labels);
        }
    ...
    fn calculate_auc(&self, scores: &[f64], labels: &[bool]) -> f64 {
        let mut score_label_pairs: Vec<(f64, bool)> = scores.iter()
            .zip(labels.iter())
            .map(|(&s, &l)| (s, l))
            .collect();
