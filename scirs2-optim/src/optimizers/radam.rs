    ...
    D: Dimension,
{
    fn step(&mut self, params: &Array<A, D>, gradients: &Array<A, D>) -> Result<Array<A, D>> {
        ...
        let rho_t = self.rho_inf
            - <A as num_traits::NumCast>::from(2.0).unwrap() * <A as num_traits::NumCast>::from(self.t as f64).unwrap() * beta2_t / (A::one() - beta2_t);
        ...
        if rho_t > <A as num_traits::NumCast>::from(4.0).unwrap() {
            ...
            let sma_rectifier =
                (rho_t - <A as num_traits::NumCast>::from(4.0).unwrap()) * (rho_t - <A as num_traits::NumCast>::from(2.0).unwrap()) / self.rho_inf;
